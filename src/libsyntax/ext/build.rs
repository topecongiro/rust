...
pub trait AstBuilder {
    ...
    fn lambda_stmts(&self, span: Span, ids: Vec<ast::Ident>, blk: Vec<ast::Stmt>) -> P<ast::Expr>;
    ...
    fn item_use(&self, sp: Span, vis: ast::Visibility, vp: P<ast::ViewPath>) -> P<ast::Item>;
...
impl<'a> AstBuilder for ExtCtxt<'a> {
    ...
    fn expr_usize(&self, span: Span, i: usize) -> P<ast::Expr> {
        self.expr_lit(
            span,
            ast::LitKind::Int(i as u128, ast::LitIntType::Unsigned(ast::UintTy::Us)),
        )
    }
    fn expr_isize(&self, sp: Span, i: isize) -> P<ast::Expr> {
        if i < 0 {
            ...
            self.expr_lit(
                sp,
                ast::LitKind::Int(i as u128, ast::LitIntType::Signed(ast::IntTy::Is)),
            )
        }
    }
    fn expr_u32(&self, sp: Span, u: u32) -> P<ast::Expr> {
    ...
    fn item_use_simple_(
        ...
        self.item_use(sp, vis, P(respan(sp, ast::ViewPathSimple(ident, path))))
    }
    ...
    fn item_use_list(
        ...
        self.item_use(
            sp,
            vis,
            P(respan(sp, ast::ViewPathList(self.path(sp, path), imports))),
        )
    }
    ...
    fn item_use_glob(&self, sp: Span, vis: ast::Visibility, path: Vec<ast::Ident>) -> P<ast::Item> {
        self.item_use(
            sp,
            vis,
            P(respan(sp, ast::ViewPathGlob(self.path(sp, path)))),
        )
    }
}
