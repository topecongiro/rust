...
pub fn walk_item<'a, V: Visitor<'a>>(visitor: &mut V, item: &'a Item) {
    ...
    match item.node {
        ...
        ItemKind::Fn(ref declaration, unsafety, constness, abi, ref generics, ref body) => {
            ...
            visitor.visit_fn(
                FnKind::ItemFn(item.ident, unsafety, constness, abi, &item.vis, body),
                declaration,
                item.span,
                item.id,
            )
...
pub fn walk_use_tree<'a, V: Visitor<'a>>(visitor: &mut V, use_tree: &'a UseTree, id: NodeId) {
    ...
    match use_tree.kind {
        ...
        UseTreeKind::Glob => {}
        UseTreeKind::Nested(ref use_trees) => for &(ref nested_tree, nested_id) in use_trees {
...
pub fn walk_expr<'a, V: Visitor<'a>>(visitor: &mut V, expression: &'a Expr) {
    ...
    match expression.node {
        ...
        ExprKind::Closure(_, _, ref function_declaration, ref body, _decl_span) => visitor
            .visit_fn(
                FnKind::Closure(body),
