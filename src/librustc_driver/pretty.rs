...
impl PpMode {
    pub fn needs_ast_map(&self, opt_uii: &Option<UserIdentifiedItem>) -> bool {
        match *self {
            PpmSource(PpmNormal) | PpmSource(PpmEveryBodyLoops) | PpmSource(PpmIdentified) => {
                opt_uii.is_some()
            }
...
impl PpSourceMode {
    ...
    where
        F: FnOnce(&HirPrinterSupport, &hir::Crate) -> A,
    {
        match *self {
            PpmNormal => {
                let annotation = NoAnn {
                ...
                abort_on_err(
                    driver::phase_3_run_analysis_passes(
                        ...
                        |tcx, _, _, _| {
                            ...
                            tcx.dep_graph
                                .with_ignore(|| f(&annotation, hir_map.forest.krate()))
                        },
                    ),
                    sess,
                )
...
trait HirPrinterSupport<'hir>: pprust_hir::PpAnn {
    ...
    fn node_path(&self, id: ast::NodeId) -> Option<String> {
        self.hir_map()
            .and_then(|map| map.def_path_from_id(id))
            .map(|path| {
                path.data
                    .into_iter()
...
impl<'hir> pprust_hir::PpAnn for NoAnn<'hir> {
    fn nested(&self, state: &mut pprust_hir::State, nested: pprust_hir::Nested) -> io::Result<()> {
...
impl UserIdentifiedItem {
    ...
    fn all_matching_node_ids<'a, 'hir>(
        &'a self,
...
fn print_flowgraph<'a, 'tcx, W: Write>(
    ...
    match code {
        ...
        blocks::Code::Expr(_) => {
            tcx.sess.err(
                "--pretty flowgraph with -Z flowgraph-print annotations requires \
                 fn-like node id.",
            );
            return Ok(());
        }
...
pub fn print_after_hir_lowering<'tcx, 'a: 'tcx>(
    sess: &'a Session,
    cstore: &'tcx CrateStore,
    hir_map: &hir_map::Map<'tcx>,
    analysis: &ty::CrateAnalysis,
...
fn print_with_analysis<'tcx, 'a: 'tcx>(
    sess: &'a Session,
    cstore: &'a CrateStore,
    hir_map: &hir_map::Map<'tcx>,
    analysis: &ty::CrateAnalysis,
    ...
    abort_on_err(
        driver::phase_3_run_analysis_passes(
            ...
            |tcx, _, _, _| match ppm {
                ...
                PpmFlowGraph(mode) => {
                    let nodeid = nodeid.expect(
                        "`pretty flowgraph=..` needs NodeId (int) or unique path \
                         suffix (b::c::d)",
                    );
                    let node = tcx.hir.find(nodeid).unwrap_or_else(|| {
                        tcx.sess
                            .fatal(&format!("--pretty flowgraph couldn't find id: {}", nodeid))
                    });
