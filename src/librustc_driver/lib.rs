...
use syntax_pos::{FileName, MultiSpan, DUMMY_SP};
...
pub fn get_trans(sess: &Session) -> Box<TransCrate> {
    ...
    INIT.call_once(|| {
        let trans_name = sess.opts
            .debugging_opts
            .codegen_backend
            .as_ref()
            .unwrap_or(&sess.target.target.options.codegen_backend);
        let backend = match &trans_name[..] {
            "metadata_only" => rustc_trans_utils::trans_crate::MetadataOnlyTransCrate::new,
...
pub fn run_compiler<'a>(
    ...
    do_or_return!(
        callbacks.late_callback(&*trans, &matches, &sess, &cstore, &input, &odir, &ofile),
        Some(sess)
    );
    ...
    fn no_input(
        ...
        match matches.free.len() {
            0 => {
                ...
                sess.parse_sess.config = cfg;
                let should_stop =
                    RustcDefaultCalls::print_crate_info(&*trans, &sess, None, odir, ofile);
