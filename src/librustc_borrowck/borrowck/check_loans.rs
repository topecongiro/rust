...
fn owned_ptr_base_path<'a, 'tcx>(loan_path: &'a LoanPath<'tcx>) -> &'a LoanPath<'tcx> {
    ...
    fn helper<'a, 'tcx>(loan_path: &'a LoanPath<'tcx>) -> Option<&'a LoanPath<'tcx>> {
        match loan_path.kind {
            ...
            LpExtend(ref lp_base, _, LpDeref(mc::Unique)) => match helper(&lp_base) {
                v @ Some(_) => v,
                None => Some(&lp_base),
            },
            LpDowncast(ref lp_base, _) | LpExtend(ref lp_base, ..) => helper(&lp_base),
        }
    }
}
...
impl<'a, 'tcx> euv::Delegate<'tcx> for CheckLoanCtxt<'a, 'tcx> {
    ...
    fn matched_pat(&mut self, _matched_pat: &hir::Pat, _cmt: &mc::cmt_, _mode: euv::MatchMode) {}
        ...
        if let Some(lp) = opt_loan_path(assignee_cmt) {
            match mode {
                ...
                MutateMode::WriteAndRead => {
                    ...
                    // we write it.
                    let hir_id = self.tcx().hir.node_to_hir_id(assignee_cmt.id);
                    self.check_if_path_is_moved(hir_id.local_id, assignment_span, MovedInUse, &lp);
                }
            }
        }
        self.check_assignment(
            self.tcx().hir.node_to_hir_id(assignment_id).local_id,
            assignment_span,
            assignee_cmt,
        );
    }
...
fn compatible_borrow_kinds(borrow_kind1: ty::BorrowKind, borrow_kind2: ty::BorrowKind) -> bool {
...
impl<'a, 'tcx> CheckLoanCtxt<'a, 'tcx> {
    ...
    pub fn each_issued_loan<F>(&self, node: hir::ItemLocalId, mut op: F) -> bool
    where
    ...
    pub fn each_in_scope_loan<F>(&self, scope: region::Scope, mut op: F) -> bool
    where
        F: FnMut(&Loan<'tcx>) -> bool,
    {
        ...
        self.each_issued_loan(scope.item_local_id(), |loan| {
            if self
                .bccx
                .region_scope_tree
                .is_subscope_of(scope, loan.kill_scope)
            {
                op(loan)
            } else {
                true
        ...
        F: FnMut(&Loan<'tcx>) -> bool,
    {
        ...
        loop {
            match loan_path.kind {
                ...
                LpDowncast(ref lp_base, _) | LpExtend(ref lp_base, ..) => {
                    loan_path = &lp_base;
                }
            }
    ...
    pub fn check_for_loans_across_yields(
        ...
        let scope = match *loan_region {
            ...
            | RegionKind::ReFree(..)
            | RegionKind::ReStatic => {
                self.bccx.tcx.sess.delay_span_bug(
                    borrow_span,
                    &format!("unexpected region for local data {:?}", loan_region),
                );
                return;
            }
        ...
        if self.bccx.region_scope_tree.containing_body(scope) != Some(body_id) {
            ...
            self.bccx
                .tcx
                .sess
                .delay_span_bug(borrow_span, "borrowing local data longer than its storage");
            return;
        }

        if let Some(yield_span) =
            self.bccx
                .region_scope_tree
                .yield_in_scope_for_expr(scope, cmt.id, self.bccx.body)
        {
            self.bccx
                .cannot_borrow_across_generator_yield(borrow_span, yield_span, Origin::Ast)
                .emit();
        }
    }
    ...
    pub fn report_error_if_loans_conflict(
        ...
        assert!(
            self.bccx
                .region_scope_tree
                .scopes_intersect(old_loan.kill_scope, new_loan.kill_scope)
        );

        let err_old_new = self
            .report_error_if_loan_conflicts_with_restriction(old_loan, new_loan, old_loan, new_loan)
            .err();
        let err_new_old = self
            .report_error_if_loan_conflicts_with_restriction(new_loan, old_loan, old_loan, new_loan)
            .err();
        ...
        for restr_path in &loan1.restricted_paths {
            ...
            let (nl, ol, new_loan_msg, old_loan_msg) = {
                if new_loan.loan_path.has_fork(&old_loan.loan_path) && common.is_some() {
                    ...
                    let ol = nl.clone();
                    let new_loan_msg = format!(
                        " (via `{}`)",
                        self.bccx.loan_path_to_string(&new_loan.loan_path)
                    );
                    let old_loan_msg = format!(
                        " (via `{}`)",
            ...
            let previous_end_span = Some(
                self.tcx().sess.codemap().end_point(
                    old_loan
                        .kill_scope
                        .span(self.tcx(), &self.bccx.region_scope_tree),
                ),
            );

            let mut err = match (new_loan.kind, old_loan.kind) {
                (ty::MutBorrow, ty::MutBorrow) => self.bccx.cannot_mutably_borrow_multiply(
                    new_loan.span,
                    &nl,
                    &new_loan_msg,
                    old_loan.span,
                ...
                (ty::UniqueImmBorrow, _) => self.bccx.cannot_uniquely_borrow_by_one_closure(
                    new_loan.span,
                    &nl,
                    &new_loan_msg,
                    old_loan.span,
            ...
            match old_loan.cause {
                euv::ClosureCapture(span) => {
                    err.span_label(
                        span,
                        format!("previous borrow occurs due to use of `{}` in closure", ol),
                    );
                }
                _ => {}
            }
            ...
            UseWhileBorrowed(loan_path, loan_span) => {
                let mut err = match move_kind {
                    move_data::Captured => {
                        ...
                        err.span_label(span, "move into closure occurs here");
                        err
                    }
                    move_data::Declared | move_data::MoveExpr | move_data::MovePat => {
                        ...
                        let mut err = self
                            .bccx
                            .cannot_move_when_borrowed(span, &desc, Origin::Ast);
                        err.span_label(
                            loan_span,
                            format!(
                                "borrow of `{}` occurs here",
    ...
    pub fn analyze_restrictions_on_use(
        ...
        self.each_in_scope_loan_affecting_path(region::Scope::Node(expr_id), use_path, |loan| {
    ...
    fn check_if_path_is_moved(
        ...
        self.move_data
            .each_move_of(id, &base_lp, |the_move, moved_lp| {
    ...
    fn check_if_assigned_path_is_moved(
        ...
        match lp.kind {
            ...
            LpDowncast(ref lp_base, _) => {
                ...
                self.check_if_assigned_path_is_moved(id, span, use_kind, lp_base);
            }
    ...
    fn check_assignment(
        ...
        if let Categorization::Local(local_id) = assignee_cmt.cat {
            ...
            self.move_data
                .each_assignment_of(assignment_id, &lp, |assign| {
    ...
    pub fn report_illegal_mutation(&self, span: Span, loan_path: &LoanPath<'tcx>, loan: &Loan) {
        self.bccx
