...
fn borrowck<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, owner_def_id: DefId) -> Rc<BorrowCheckResult> {
    debug!("borrowck(body_owner_def_id={:?})", owner_def_id);

    let owner_id = tcx.hir.as_local_node_id(owner_def_id).unwrap();

    match tcx.hir.get(owner_id) {
        hir_map::NodeStructCtor(_) | hir_map::NodeVariant(_) => {
    ...
    // by mut-ref.
    let mut cfg = None;
    if let Some(AnalysisData {
        all_loans,
        loans: loan_dfcx,
        move_data: flowed_moves,
    }) = build_borrowck_dataflow_data(&mut bccx, false, body_id, |bccx| {
    ...
    F: FnOnce(&mut BorrowckCtxt<'a, 'tcx>) -> &'c cfg::CFG,
{
    ...
    let (all_loans, move_data) = gather_loans::gather_loans_in_fn(this, body_id);
    ...
    let mut loan_dfcx = DataFlowContext::new(
        this.tcx,
        "borrowck",
        Some(this.body),
        cfg,
    ...
    let flowed_moves = move_data::FlowedMoveData::new(move_data, this, cfg, id_range, this.body);
...
impl<'b, 'tcx: 'b> BorrowckErrors for BorrowckCtxt<'b, 'tcx> {
    fn struct_span_err_with_code<'a, S: Into<MultiSpan>>(
        &'a self,
        sp: S,
        msg: &str,
        code: DiagnosticId,
    ...
    fn struct_span_err<'a, S: Into<MultiSpan>>(
        &'a self,
        sp: S,
        msg: &str,
    ) -> DiagnosticBuilder<'a> {
...
fn closure_to_block(closure_id: LocalDefId, tcx: TyCtxt) -> ast::NodeId {
    ...
    match tcx.hir.get(closure_id) {
        hir_map::NodeExpr(expr) => match expr.node {
            hir::ExprClosure(.., body_id, _, _) => body_id.node_id,
...
impl<'a, 'tcx> LoanPath<'tcx> {
    pub fn kill_scope(&self, bccx: &BorrowckCtxt<'a, 'tcx>) -> region::Scope {
        match self.kind {
            ...
            LpDowncast(ref base, _) | LpExtend(ref base, ..) => base.kill_scope(bccx),
        }
    }
...
pub fn opt_loan_path<'tcx>(cmt: &mc::cmt<'tcx>) -> Option<Rc<LoanPath<'tcx>>> {
    ...
    match cmt.cat {
        ...
        Categorization::Local(id) => Some(new_lp(LpVar(id))),

        Categorization::Upvar(mc::Upvar { id, .. }) => Some(new_lp(LpUpvar(id))),

        Categorization::Deref(ref cmt_base, pk) => {
            opt_loan_path(cmt_base).map(|lp| new_lp(LpExtend(lp, cmt.mutbl, LpDeref(pk))))
        }

        Categorization::Interior(ref cmt_base, ik) => opt_loan_path(cmt_base).map(|lp| {
            ...
            new_lp(LpExtend(
                lp,
                cmt.mutbl,
                LpInterior(opt_variant_id, ik.cleaned()),
            ))
        }),

        Categorization::Downcast(ref cmt_base, variant_def_id) => {
            opt_loan_path(cmt_base).map(|lp| new_lp(LpDowncast(lp, variant_def_id)))
        }
    }
}
...
impl<'a, 'tcx> BorrowckCtxt<'a, 'tcx> {
    pub fn is_subregion_of(&self, r_sub: ty::Region<'tcx>, r_sup: ty::Region<'tcx>) -> bool {
    ...
    pub fn report_use_of_moved_value(
        ...
        let (_ol, _moved_lp_msg, mut err, need_note) = match the_move.kind {
            ...
            _ => {
                ...
                let (nl, ol, moved_lp_msg) = if has_fork && has_common {
                    let nl = self.loan_path_to_string(&common.unwrap());
                    let ol = nl.clone();
                    let moved_lp_msg =
                        format!(" (through moving `{}`)", self.loan_path_to_string(moved_lp));
                    (nl, ol, moved_lp_msg)
                } else {
                    (
                        self.loan_path_to_string(lp),
                ...
                let msg = if !has_fork && partial {
                    "partially "
                } else if has_fork && !has_common {
                    "collaterally "
                } else {
                    ""
                };
        ...
        let (move_span, move_note) = match the_move.kind {
            ...
            move_data::MoveExpr | move_data::MovePat => (self.tcx.hir.span(node_id), ""),

            move_data::Captured => (
                match self.tcx.hir.expect_expr(node_id).node {
                    hir::ExprClosure(.., fn_decl_span, _) => fn_decl_span,
                    ref r => bug!("Captured({:?}) maps to non-closure: {:?}", the_move.id, r),
                },
                " (into closure)",
            ),
        };
        ...
        err = if use_span == move_span {
            ...
            err.span_label(
                use_span,
                format!("value {} here after move", verb_participle),
    ...
    pub fn report_partial_reinitialization_of_uninitialized_structure(
        ...
        self.cannot_partially_reinit_an_uninit_struct(
            span,
            &self.loan_path_to_string(lp),
            Origin::Ast,
        ).emit();
    ...
    pub fn report_reassigned_immutable_variable(
        ...
        let mut err =
            self.cannot_reassign_immutable(span, &self.loan_path_to_string(lp), Origin::Ast);
        err.span_label(span, "cannot assign twice to immutable variable");
        if span != assign.span {
            err.span_label(
                assign.span,
                format!("first assignment to `{}`", self.loan_path_to_string(lp)),
            );
        }
        err.emit();
    }
    ...
    pub fn span_err_with_code<S: Into<MultiSpan>>(&self, s: S, msg: &str, code: DiagnosticId) {
        ...
        match err.code {
            err_mutbl => {
                let descr = match err.cmt.note {
                    mc::NoteClosureEnv(_) | mc::NoteUpvarRef(_) => self.cmt_to_string(&err.cmt),
            ...
            err_out_of_scope(super_scope, sub_scope, cause) => {
                ...
                debug!(
                    "err_out_of_scope: self.body.is_generator = {:?}",
                ...
                let (value_kind, value_msg) = match err.cmt.cat {
                    ...
                    _ => ("borrowed value", "borrow occurs here"),
                };
                ...
                match (sub_span, super_span) {
                    (Some(s1), Some(s2)) if s1 == s2 => {
                        if !is_closure {
                            ...
                            db.span_label(s1, format!("{} dropped here while still borrowed", msg));
                        } else {
                            db.span_label(s1, format!("{} dropped before borrower", value_kind));
                        }
                        db.note(
                            "values in a scope are dropped in the opposite order \
                             they are created",
                        );
                    }
                    ...
                    _ => {
                        match sub_span {
                            Some(s) => {
                                db.span_label(
                                    s,
                                    format!("{} needs to live until here", value_kind),
                                );
                            }
                            None => {
    ...
    pub fn report_aliasability_violation(
        ...
        let prefix = match kind {
            ...
            | BorrowViolation(euv::AutoUnsafe)
            | BorrowViolation(euv::RefBinding)
            | BorrowViolation(euv::MatchDiscriminant) => "cannot borrow data mutably",
            ...
            BorrowViolation(euv::ForLoop) => "`for` loop",
        };

        match cause {
            mc::AliasableStatic | mc::AliasableStaticMut => {
        ...
        let mut err = match blame {
            Some(ImmutabilityBlame::ClosureEnv(id)) => {
                ...
                self.cannot_act_on_capture_in_sharable_fn(
                    span,
                    prefix,
                    (help_span, help_msg),
                    Origin::Ast,
            ...
            _ => self.cannot_assign_into_immutable_reference(span, prefix, Origin::Ast),
        };
    ...
    fn suggest_mut_for_immutable(&self, pty: &hir::Ty, is_implicit_self: bool) -> Option<String> {
        ...
        ) = pty.node
        {
            ...
            let lifetime_snippet = if !lifetime.is_elided() {
                format!(
                    "{} ",
                    match self.tcx.sess.codemap().span_to_snippet(lifetime.span) {
    ...
    fn local_binding_mode(&self, node_id: ast::NodeId) -> ty::BindingMode {
        ...
        match pat.node {
            hir::PatKind::Binding(..) => *self.tables
                .pat_binding_modes()
                .get(pat.hir_id)
    ...
    fn local_ty(&self, node_id: ast::NodeId) -> (Option<&hir::Ty>, bool) {
        ...
        if let Some(fn_like) = FnLikeNode::from_node(parent_node) {
            ...
            let arg_pos = fn_body
                .arguments
                .iter()
                .position(|arg| arg.pat.id == node_id);
    ...
    fn report_out_of_scope_escaping_closure_capture(
        ...
        let suggestion = match self.tcx.sess.codemap().span_to_snippet(err.span) {
    ...
    fn region_end_span(&self, region: ty::Region<'tcx>) -> Option<Span> {
        match *region {
            ty::ReScope(scope) => Some(scope.span(self.tcx, &self.region_scope_tree).end_point()),
            _ => None,
        }
    }
        ...
        match err.cmt.note {
            mc::NoteClosureEnv(upvar_id) | mc::NoteUpvarRef(upvar_id) => {
                ...
                if kind == ty::ClosureKind::Fn {
                    let closure_node_id = self.tcx
                        .hir
                        .local_def_id_to_node_id(upvar_id.closure_expr_id);
                    db.span_help(
                        self.tcx.hir.span(closure_node_id),
            ...
            _ => {
                if let Categorization::Deref(..) = err.cmt.cat {
                    ...
                    if let mc::MutabilityCategory::McImmutable = cmt.mutbl {
                        db.span_label(*error_span, "cannot mutably borrow immutable field");
                    }
                }
            }
        }
    }
    pub fn append_loan_path_to_string(&self, loan_path: &LoanPath<'tcx>, out: &mut String) {
...
impl<'tcx> fmt::Debug for LoanPath<'tcx> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.kind {
            LpVar(id) => write!(f, "$({})", ty::tls::with(|tcx| tcx.hir.node_to_string(id))),
            ...
            LpExtend(ref lp, _, LpDeref(_)) => write!(f, "{:?}.*", lp),
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.kind {
            LpVar(id) => write!(
                f,
                "$({})",
                ty::tls::with(|tcx| tcx.hir.node_to_user_string(id))
            ),
            ...
            LpExtend(ref lp, _, LpInterior(_, ref interior)) => write!(f, "{}.{:?}", lp, interior),
        }
    }
}
