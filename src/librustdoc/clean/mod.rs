...
use syntax_pos::{self, FileName, Pos, DUMMY_SP};

use rustc::hir::def::{self, CtorKind, Def};
...
use rustc::ty::{self, AdtKind, GenericParamCount, Region, RegionVid, Ty, TyCtxt};
...
impl<'a, 'tcx, 'rcx> Clean<Crate> for visit_ast::RustdocVisitor<'a, 'tcx, 'rcx> {
    fn clean(&self, cx: &DocContext) -> Crate {
        ...
        } = LOCAL_CRATE.clean(cx);
        {
            ...
            m.items
                .extend(primitives.iter().map(|&(def_id, prim, ref attrs)| Item {
                    source: Span::empty(),
            ...
            m.items
                .extend(keywords.into_iter().map(|(def_id, kw, attrs)| Item {
                    source: Span::empty(),
                    name: Some(kw.clone()),
                    attrs: attrs,
                    visibility: Some(Public),
        ...
        let primitives = if root.is_local() {
            cx.tcx
                ...
                .filter_map(|&id| {
                    ...
                    match item.node {
                        hir::ItemMod(_) => as_primitive(Def::Mod(cx.tcx.hir.local_def_id(id.id))),
                ...
                for attr in attrs.lists("doc") {
                    if let Some(v) = attr.value_str() {
                        if attr.check_name("keyword") {
                            keyword = Keyword::from_str(&v.as_str())
                                .ok()
        ...
        let keywords = if root.is_local() {
            cx.tcx
                ...
                .filter_map(|&id| {
                    let item = cx.tcx.hir.expect_item(id.id);
                    match item.node {
                        hir::ItemMod(_) => as_keyword(Def::Mod(cx.tcx.hir.local_def_id(id.id))),
                        hir::ItemUse(ref path, hir::UseKind::Single)
                            if item.vis == hir::Visibility::Public =>
                        {
...
impl Item {
    ...
    pub fn has_stripped_fields(&self) -> Option<bool> {
        match self.inner {
            ...
            VariantItem(Variant {
                kind: VariantKind::Struct(ref vstruct),
...
impl<I: IntoIterator<Item = ast::NestedMetaItem>> NestedAttributesExt for I {
    fn has_word(self, word: &str) -> bool {
        self.into_iter()
            .any(|attr| attr.is_word() && attr.check_name(word))
    }
}
...
impl Attributes {
    ...
    /// its expansion.
    fn extract_include(mi: &ast::MetaItem) -> Option<(String, String)> {
    ...
    pub fn has_doc_flag(&self, flag: &str) -> bool {
        for attr in &self.other_attrs {
            ...
            if let Some(items) = attr.meta_item_list() {
                if items
                    .iter()
                    .filter_map(|i| i.meta_item())
                    .any(|it| it.check_name(flag))
                {
                    return true;
                }
            }
...
fn value_ns_kind(def: Def, path_str: &str) -> Option<(&'static str, String)> {
    match def {
        ...
        Def::Fn(..) => Some(("function", format!("{}()", path_str))),
        Def::Method(..) => Some(("method", format!("{}()", path_str))),
        Def::Const(..) => Some(("const", format!("const@{}", path_str))),
        Def::Static(..) => Some(("static", format!("static@{}", path_str))),
...
fn ambiguity_error(
    ...
    cx.sess()
        .struct_span_warn(
            ...
            &format!(
                "`{}` is both {} {} and {} {}",
                path_str, article1, kind1, article2, kind2
            ),
        )
        .help(&format!(
            "try `{}` if you want to select the {}, \
             or `{}` if you want to \
             select the {}",
            disambig1, kind1, disambig2, kind2
        ))
        .emit();
}
...
const PRIMITIVES: &[(&str, Def)] = &[
    (
        "u8",
        Def::PrimTy(hir::PrimTy::TyUint(syntax::ast::UintTy::U8)),
    ),
    (
        "u16",
        Def::PrimTy(hir::PrimTy::TyUint(syntax::ast::UintTy::U16)),
    ),
    (
        "u32",
        Def::PrimTy(hir::PrimTy::TyUint(syntax::ast::UintTy::U32)),
    ),
    (
        "u64",
        Def::PrimTy(hir::PrimTy::TyUint(syntax::ast::UintTy::U64)),
    ),
    (
        "u128",
        Def::PrimTy(hir::PrimTy::TyUint(syntax::ast::UintTy::U128)),
    ),
    (
        "usize",
        Def::PrimTy(hir::PrimTy::TyUint(syntax::ast::UintTy::Usize)),
    ),
    (
        "i8",
        Def::PrimTy(hir::PrimTy::TyInt(syntax::ast::IntTy::I8)),
    ),
    (
        "i16",
        Def::PrimTy(hir::PrimTy::TyInt(syntax::ast::IntTy::I16)),
    ),
    (
        "i32",
        Def::PrimTy(hir::PrimTy::TyInt(syntax::ast::IntTy::I32)),
    ),
    (
        "i64",
        Def::PrimTy(hir::PrimTy::TyInt(syntax::ast::IntTy::I64)),
    ),
    (
        "i128",
        Def::PrimTy(hir::PrimTy::TyInt(syntax::ast::IntTy::I128)),
    ),
    (
        "isize",
        Def::PrimTy(hir::PrimTy::TyInt(syntax::ast::IntTy::Isize)),
    ),
    (
        "f32",
        Def::PrimTy(hir::PrimTy::TyFloat(syntax::ast::FloatTy::F32)),
    ),
    (
        "f64",
        Def::PrimTy(hir::PrimTy::TyFloat(syntax::ast::FloatTy::F64)),
    ),
...
fn resolve(cx: &DocContext, path_str: &str, is_val: bool) -> Result<(Def, Option<String>), ()> {
    ...
    if let Some(id) = cx.mod_ids.borrow().last() {
        let result = cx.resolver.borrow_mut().with_scope(*id, |resolver| {
        ...
        let ty = cx.resolver.borrow_mut().with_scope(*id, |resolver| {
...
fn macro_resolve(cx: &DocContext, path_str: &str) -> Option<Def> {
    ...
    let mark = Mark::root();
    let res = resolver.resolve_macro_to_def_inner(mark, &path, MacroKind::Bang, false);
    if let Ok(def) = res {
        ...
        if UnstableFeatures::from_environment().is_nightly_build() {
            ...
            for (ori_link, link_range) in markdown_links(&dox) {
                ...
                let (def, fragment) = {
                    ...
                    if path_str
                        .contains(|ch: char| !(ch.is_alphanumeric() || ch == ':' || ch == '_'))
                    {
                        continue;
                    }
                            ...
                            if let Some(macro_def) = macro_resolve(cx, path_str) {
                                if let Ok(type_def) = resolve(cx, path_str, false) {
                                    ...
                                    ambiguity_error(
                                        ...
                                        "a",
                                        "macro",
                                        &format!("macro@{}", path_str),
                                    );
                                    continue;
                                } else if let Ok(value_def) = resolve(cx, path_str, true) {
                                    let (value_kind, value_disambig) =
                                        value_ns_kind(value_def.0, path_str).expect(
                                            "struct and mod cases should have been \
...
fn external_path_params(
    ...
    match trait_did {
        ...
        _ => PathParameters::AngleBracketed {
            lifetimes,
            types: types.clean(cx),
            bindings,
        },
...
impl<'a, 'tcx> Clean<TyParamBound> for (&'a ty::TraitRef<'tcx>, Vec<TypeBinding>) {
    fn clean(&self, cx: &DocContext) -> TyParamBound {
        ...
        let path = external_path(
            cx,
            &cx.tcx.item_name(trait_ref.def_id).as_str(),
...
impl<'tcx> Clean<Option<Vec<TyParamBound>>> for Substs<'tcx> {
    fn clean(&self, cx: &DocContext) -> Option<Vec<TyParamBound>> {
        let mut v = Vec::new();
        v.extend(self.regions().filter_map(|r| r.clean(cx)).map(RegionBound));
...
impl<'a, 'tcx> Clean<Generics> for (&'a ty::Generics, &'a ty::GenericPredicates<'tcx>) {
    fn clean(&self, cx: &DocContext) -> Generics {
    ...
    fn clean(&self, cx: &DocContext) -> Item {
        let inner = match self.node {
            hir::TraitItemKind::Const(ref ty, default) => {
                AssociatedConstItem(ty.clean(cx), default.map(|e| print_const_expr(cx, e)))
            }
    ...
    fn clean(&self, cx: &DocContext) -> Item {
        let inner = match self.node {
            hir::ImplItemKind::Const(ref ty, expr) => {
                AssociatedConstItem(ty.clean(cx), Some(print_const_expr(cx, expr)))
            }
            hir::ImplItemKind::Method(ref sig, body) => {
                MethodItem((sig, &self.generics, body).clean(cx))
            }
            hir::ImplItemKind::Type(ref ty) => TypedefItem(
                Typedef {
                    type_: ty.clean(cx),
    ...
    fn clean(&self, cx: &DocContext) -> Item {
        let inner = match self.kind {
            ...
            ty::AssociatedKind::Method => {
                let generics = (
                    cx.tcx.generics_of(self.def_id),
                    &cx.tcx.predicates_of(self.def_id),
                ).clean(cx);
                        ...
                        for param in generics.params.iter() {
                            match param {
                                hir::GenericParam::Lifetime(lt_param) => {
                                    if let Some(lt) =
                                        provided_params.lifetimes.get(indices.lifetimes).cloned()
                                    {
                                        if !lt.is_elided() {
                                            let lt_def_id =
                                ...
                                hir::GenericParam::Type(ty_param) => {
                                    ...
                                    if let Some(ty) =
                                        provided_params.types.get(indices.types).cloned()
                                    {
                                        ty_substs.insert(ty_param_def, ty.into_inner().clean(cx));
                                    } else if let Some(default) = ty_param.default.clone() {
                                        ty_substs
                                            .insert(ty_param_def, default.into_inner().clean(cx));
                                    }
                                    indices.types += 1;
                                }
                            }
            ...
            TyTraitObject(ref bounds, ref lifetime) => {
                match bounds[0].clean(cx).trait_ {
                    ResolvedPath {
                        ...
                        let mut bounds: Vec<_> = bounds[1..]
                            .iter()
                            .map(|bound| TraitBound(bound.clean(cx), hir::TraitBoundModifier::None))
                            .collect();
                    ...
                    .filter_map(|predicate| {
                        let trait_ref = if let Some(tr) = predicate.to_opt_poly_trait_ref() {
                            ...
                            pred.skip_binder()
                                .1
                                .clean(cx)
                                .map(|r| regions.push(RegionBound(r)));
                            return None;
                        } else {
                            return None;
                        };
...
impl Clean<Option<Visibility>> for ty::Visibility {
    fn clean(&self, _: &DocContext) -> Option<Visibility> {
        Some(if *self == ty::Visibility::Public {
            Public
        } else {
            Inherited
        })
    ...
    fn clean(&self, cx: &DocContext) -> PathParameters {
        if self.parenthesized {
            ...
            PathParameters::Parenthesized {
                ...
                output: if output != Type::Tuple(Vec::new()) {
                    Some(output)
                } else {
                    None
                },
...
fn strip_type(ty: Type) -> Type {
    match ty {
        ...
        Type::BorrowedRef {
            ...
            lifetime,
            mutability,
            type_: Box::new(strip_type(*type_)),
        },
        Type::QPath {
            name,
            self_type,
...
fn build_deref_target_impls(cx: &DocContext, items: &[Item], ret: &mut Vec<Item>) {
    use self::PrimitiveType::*;
    let tcx = cx.tcx;
    ...
    fn clean(&self, cx: &DocContext) -> Vec<Item> {
        ...
        let denied = self.vis != hir::Public || self.attrs.iter().any(|a| {
            a.name() == "doc" && match a.meta_item_list() {
                Some(l) => {
                    attr::list_contains_name(&l, "no_inline")
...
impl Clean<Item> for hir::ForeignItem {
    fn clean(&self, cx: &DocContext) -> Item {
        let inner = match self.node {
            ...
            hir::ForeignItemStatic(ref ty, mutbl) => ForeignStaticItem(Static {
                type_: ty.clean(cx),
                mutability: if mutbl { Mutable } else { Immutable },
                expr: "".to_string(),
            }),
            hir::ForeignItemType => ForeignTypeItem,
        };
        Item {
            name: Some(self.name.clean(cx)),
            attrs: self.attrs.clean(cx),
...
fn name_from_pat(p: &hir::Pat) -> String {
    ...
    match p.node {
        ...
        PatKind::Struct(ref name, ref fields, etc) => format!(
            ...
            fields
                .iter()
                .map(|&Spanned { node: ref fp, .. }| format!(
                    "{}: {}",
                    fp.ident,
                    name_from_pat(&*fp.pat)
                ))
                .collect::<Vec<String>>()
                .join(", "),
            if etc { ", ..." } else { "" }
        ...
        PatKind::Slice(ref begin, ref mid, ref end) => {
            ...
            let mid = mid
                .as_ref()
                .map(|p| format!("..{}", name_from_pat(&**p)))
                .into_iter();
            let end = end.iter().map(|p| name_from_pat(&**p));
            format!(
                "[{}]",
                begin.chain(mid).chain(end).collect::<Vec<_>>().join(", ")
            )
        }
    }
}
...
fn resolve_type(cx: &DocContext, path: Path, id: ast::NodeId) -> Type {
    ...
    F: Fn(DefId) -> Def,
{
    ...
    hir::Path {
        span: DUMMY_SP,
        def: def_ctor(def_id),
        segments: hir::HirVec::from_vec(
            apb.names
                .iter()
                .map(|s| hir::PathSegment {
    ...
    fn from(bound: TyParamBound) -> Self {
        match bound.clone() {
            ...
            TyParamBound::TraitBound(t, mod_) => match t.trait_ {
                Type::ResolvedPath { path, typarams, .. } => SimpleBound::TraitBound(
                    path.segments,
                    typarams.map_or_else(
                        || Vec::new(),
