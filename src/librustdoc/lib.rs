...
use rustc::session::config::{nightly_options, ErrorOutputType, Externs, RustcOptGroup};
    ...
    vec![
        ...
        unstable("disable-commonmark", |o| {
            o.optflag(
                "",
                "disable-commonmark",
                "to disable commonmark doc rendering/testing",
            )
        }),
        ...
        unstable("deny-render-differences", |o| {
            o.optflag(
                ...
                "abort doc runs when markdown rendering \
...
fn print_error<T>(error_message: T)
where
    T: Display,
{
    writeln!(
