...
use rustc::session::config::{build_codegen_options, nightly_options};
use rustc::session::config::{CodegenOptions, ErrorOutputType, Externs, RustcOptGroup};
...
pub fn main() {
    ...
    let res = std::thread::Builder::new()
        ...
        .spawn(move || {
            syntax::with_globals(move || get_args().map(|args| main_args(&args)).unwrap_or(1))
        })
        .unwrap()
        .join()
        .unwrap_or(101);
...
pub fn opts() -> Vec<RustcOptGroup> {
    vec![
        ...
        stable("V", |o| {
            o.optflag("V", "version", "print rustdoc's version")
        }),
        ...
        stable("o", |o| {
            o.optopt("o", "output", "where to place the output", "PATH")
        }),
        stable("crate-name", |o| {
        ...
        stable("test", |o| {
            o.optflag("", "test", "run code examples as tests")
        }),
        stable("test-args", |o| {
            o.optmulti(
                "",
        ...
        stable("target", |o| {
            o.optopt("", "target", "target triple to document", "TRIPLE")
        }),
        ...
        unstable("Z", |o| {
            o.optmulti(
                "Z",
                "",
                "internal and debugging options (only on nightly build)",
                "FLAG",
            )
        }),
        stable("sysroot", |o| {
        ...
        unstable("linker", |o| {
            o.optopt(
                "",
                "linker",
                "linker used for building executable test code",
                "PATH",
            )
        }),
...
pub fn main_args(args: &[String]) -> isize {
    ...
    let markdown_input = Path::new(input)
        .extension()
...
fn parse_externs(matches: &getopts::Matches) -> Result<Externs, String> {
    ...
    for arg in &matches.opt_strs("extern") {
        ...
        let location = parts
            .next()
            .ok_or("--extern value must be of the format `foo=bar`".to_string())?;
    ...
    F: 'static + Send + FnOnce(Output) -> R,
{
    ...
    let force_unstable_if_unmarked = matches
        .opt_strs("Z")
        .iter()
        .any(|x| *x == "force-unstable-if-unmarked");
    ...
    rustc_driver::monitor(move || {
        syntax::with_globals(move || {
            ...
            let (mut krate, renderinfo) = core::run_core(
                paths,
                cfgs,
                externs,
                Input::File(cratefile),
                triple,
                maybe_sysroot,
                display_warnings,
                crate_name.clone(),
            ...
            for pass in &passes {
                let plugin = match passes::PASSES.iter().position(|&(p, ..)| p == *pass) {
