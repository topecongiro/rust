...
pub fn render_with_highlighting(
    ...
    let fm = sess.codemap()
        .new_filemap(FileName::Custom("stdin".to_string()), src.to_string());
...
impl<'a> Classifier<'a> {
    ...
    fn write_token<W: Writer>(&mut self, out: &mut W, tas: TokenAndSpan) -> io::Result<()> {
        let klass = match tas.tok {
            ...
            token::Ident(ident) => match &*ident.name.as_str() {
...
fn write_header(class: Option<&str>, id: Option<&str>, out: &mut Write) -> io::Result<()> {
    write!(out, "<pre ")?;
    if let Some(id) = id {
        write!(out, "id='{}' ", id)?;
    }
