...
pub trait TransItemExt<'a, 'tcx>: fmt::Debug {
    ...
    fn instantiation_mode(&self, tcx: TyCtxt<'a, 'tcx, 'tcx>) -> InstantiationMode {
        ...
        match *self.as_trans_item() {
            TransItem::Fn(ref instance) => {
                ...
                if self.explicit_linkage(tcx).is_some() || !common::requests_inline(tcx, instance) {
    ...
    fn to_string(&self, tcx: TyCtxt<'a, 'tcx, 'tcx>) -> String {
        ...
        return match *self.as_trans_item() {
            TransItem::Fn(instance) => to_string_internal(tcx, "fn ", instance),
            TransItem::Static(node_id) => {
                let def_id = hir_map.local_def_id(node_id);
                let instance = Instance::new(def_id, tcx.intern_substs(&[]));
                to_string_internal(tcx, "static ", instance)
            }
            TransItem::GlobalAsm(..) => "global_asm".to_string(),
        };
