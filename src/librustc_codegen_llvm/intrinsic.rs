...
pub fn codegen_intrinsic_call<'a, 'tcx>(
    bx: &Builder<'a, 'tcx>,
    callee_ty: Ty<'tcx>,
    ...
    let llval = match name {
        ...
        "size_of_val" => {
            ...
            if let OperandValue::Pair(_, meta) = args[0].val {
                let (llsize, _) = glue::size_and_align_of_dst(bx, tp_ty, meta);
                llsize
            } else {
                C_usize(cx, cx.size_of(tp_ty).bytes())
            }
        ...
        "min_align_of_val" => {
            let tp_ty = substs.type_at(0);
            if let OperandValue::Pair(_, meta) = args[0].val {
                let (_, llalign) = glue::size_and_align_of_dst(bx, tp_ty, meta);
                llalign
            } else {
                C_usize(cx, cx.align_of(tp_ty).abi())
            }
        ...
        "type_id" => C_u64(cx, cx.tcx.type_id_hash(substs.type_at(0))),
        "init" => {
        ...
        "copy_nonoverlapping" => copy_intrinsic(
            bx,
            false,
            false,
            substs.type_at(0),
        ...
        "volatile_copy_nonoverlapping_memory" => copy_intrinsic(
            bx,
            false,
            true,
            substs.type_at(0),
        ...
        "volatile_copy_memory" => copy_intrinsic(
            bx,
            true,
            true,
            substs.type_at(0),
        ...
        "volatile_set_memory" => memset_intrinsic(
            bx,
            true,
            substs.type_at(0),
            args[0].immediate(),
        ...
        "ctlz" | "ctlz_nonzero" | "cttz" | "cttz_nonzero" | "ctpop" | "bswap" | "bitreverse"
        | "add_with_overflow" | "sub_with_overflow" | "mul_with_overflow" | "overflowing_add"
        | "overflowing_sub" | "overflowing_mul" | "unchecked_div" | "unchecked_rem"
        | "unchecked_shl" | "unchecked_shr" | "exact_div" => {
            let ty = arg_tys[0];
                ...
                Some((width, signed)) => match name {
                    ...
                    "bswap" => {
                        if width == 8 {
                            ...
                            bx.call(
                                cx.get_intrinsic(&format!("llvm.bswap.i{}", width)),
                                &[args[0].immediate()],
                                None,
                            )
                        }
                    ...
                    "bitreverse" => bx.call(
                        cx.get_intrinsic(&format!("llvm.bitreverse.i{}", width)),
                        &[args[0].immediate()],
                        None,
                    ),
                    "add_with_overflow" | "sub_with_overflow" | "mul_with_overflow" => {
                        ...
                        let pair = bx.call(llfn, &[args[0].immediate(), args[1].immediate()], None);
        ...
        "discriminant_value" => args[0].deref(bx.cx).codegen_get_discr(bx, ret_ty),

        name if name.starts_with("simd_") => {
            match generic_simd_intrinsic(bx, name, callee_ty, args, ret_ty, llret_ty, span) {
                Ok(llval) => llval,
                Err(()) => return,
            }
        }
        ...
        _ => {
            ...
            fn ty_to_type(cx: &CodegenCx, t: &intrinsics::Type) -> Vec<Type> {
                ...
                match *t {
                    ...
                    Integer(_signed, _width, llvm_width) => vec![Type::ix(cx, llvm_width as u64)],
                    Float(x) => match x {
                        32 => vec![Type::f32(cx)],
                        64 => vec![Type::f64(cx)],
                        _ => bug!(),
                    ...
                    Aggregate(true, ref contents) => {
                        contents.iter().flat_map(|t| ty_to_type(cx, t)).collect()
                    }
                }
            }
            ...
            fn modify_as_needed<'a, 'tcx>(
                ...
                match *t {
                    intrinsics::Type::Aggregate(true, ref contents) => {
                        ...
                        (0..contents.len())
                            .map(|i| arg.project_field(bx, i).load(bx).immediate())
                            .collect()
                    }
            ...
            let llargs: Vec<_> = intr
                .inputs
                .iter()
                .zip(args)
                .flat_map(|(t, arg)| modify_as_needed(bx, t, arg))
                .collect();
            assert_eq!(inputs.len(), llargs.len());

            let val = match intr.definition {
                intrinsics::IntrinsicDef::Named(name) => {
                    let f = declare::declare_cfn(cx, name, Type::func(&inputs, &outputs));
...
fn copy_intrinsic<'a, 'tcx>(
    ...
    let operation = if allow_overlap { "memmove" } else { "memcpy" };
...
fn codegen_gnu_try<'a, 'tcx>(
    ...
    let llfn = get_rust_try_fn(cx, &mut |bx| {
        ...
        let ptr_align = bx.tcx().data_layout.pointer_align;
            ...
            ptr,
            catch.bitcast(local_ptr, Type::i8p(cx).ptr_to()),
            ptr_align,
        );
...
fn generic_simd_intrinsic<'a, 'tcx>(
    ...
    macro_rules! require_simd {
        ($ty:expr, $position:expr) => {
            require!(
                $ty.is_simd(),
                "expected SIMD {} type, found non-SIMD `{}`",
                $position,
                $ty
            )
        };
    ...
    if let Some(cmp_op) = comparison {
        ...
        require!(
            llret_ty.element_type().kind() == llvm::Integer,
    ...
    if name.starts_with("simd_shuffle") {
        ...
        return Ok(bx.shuffle_vector(args[0].immediate(), args[1].immediate(), C_vector(&indices)));
    }
    ...
    fn simd_simple_float_intrinsic<'a, 'tcx>(
        name: &str,
    ...
    if name == "simd_scatter" {
        ...
        let f = declare::declare_cfn(
            ...
            &llvm_intrinsic,
            Type::func(
                &[llvm_elem_vec_ty, llvm_pointer_vec_ty, alignment_ty, mask_ty],
                &ret_t,
            ),
        );
        llvm::SetUnnamedAddr(f, false);
                    ...
                    ty::TyFloat(f) => {
                        ...
                        let acc = if $ordered {
                            ...
                            match const_get_real(acc) {
                                None => {
                                    return_error!("accumulator of {} is not a constant", $name)
                                }
                                Some((v, loses_info)) => {
                                    if $name.contains("mul") && v != 1.0_f64 {
                                        ...
                                        return_error!(
                                            "accumulator of {} loses information",
                                            $name
                                        );
                                    }
                                }
...
unsupported {} from `{}` with element `{}` of size `{}` to `{}`"#,
    ...
    arith_red!(
        "simd_reduce_add_ordered": vector_reduce_add,
        vector_reduce_fadd_fast,
        true
    );
    arith_red!(
        "simd_reduce_mul_ordered": vector_reduce_mul,
    ...
    macro_rules! minmax_red {
        ($name:tt : $int_red:ident, $float_red:ident) => {
            if name == $name {
                ...
                return match in_elem.sty {
                    ty::TyInt(_i) => Ok(bx.$int_red(args[0].immediate(), true)),
                    ty::TyUint(_u) => Ok(bx.$int_red(args[0].immediate(), false)),
                    ty::TyFloat(_f) => Ok(bx.$float_red(args[0].immediate())),
                    _ => return_error!(
                        "unsupported {} from `{}` with element `{}` to `{}`",
                        $name,
                        in_ty,
    ...
    minmax_red!(
        "simd_reduce_min_nanless": vector_reduce_min,
        vector_reduce_fmin_fast
    );
    minmax_red!(
        "simd_reduce_max_nanless": vector_reduce_max,
...
fn int_type_width_signed(ty: Ty, cx: &CodegenCx) -> Option<(u64, bool)> {
    match ty.sty {
        ty::TyInt(t) => Some((
            match t {
                ast::IntTy::Isize => match &cx.tcx.sess.target.target.target_pointer_width[..] {
                    "16" => 16,
                    "32" => 32,
                    "64" => 64,
        ...
        ty::TyUint(t) => Some((
            match t {
                ast::UintTy::Usize => match &cx.tcx.sess.target.target.target_pointer_width[..] {
                    "16" => 16,
                    "32" => 32,
                    "64" => 64,
