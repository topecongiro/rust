...
use rustc::session::config::{self, CrateType, CrossLangLto, DebugInfoLevel, OptLevel};
...
impl LinkerInfo {
    ...
    pub fn to_linker<'a>(&'a self, cmd: Command, sess: &'a Session) -> Box<Linker + 'a> {
        match sess.linker_flavor() {
            ...
            LinkerFlavor::Lld(LldFlavor::Wasm) => Box::new(WasmLd { cmd }) as Box<Linker>,
        }
    }
}
...
impl<'a> Linker for GccLinker<'a> {
    ...
    fn cross_lang_lto(&mut self) {
        match self.sess.opts.debugging_opts.cross_lang_lto {
            CrossLangLto::Disabled | CrossLangLto::NoLink => {
                // Nothing to do
            }
            CrossLangLto::LinkerPlugin(ref path) => {
                ...
                match self.sess.opts.cg.lto {
                    config::Lto::Thin | config::Lto::ThinLocal => {
                        self.linker_arg(&format!("-plugin-opt=thin"));
                    }
                    config::Lto::Fat | config::Lto::Yes | config::Lto::No => {
