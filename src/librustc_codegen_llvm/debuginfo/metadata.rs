...
fn fixed_vec_metadata<'a, 'tcx>(
    ...
    let upper_bound = match array_or_slice_type.sty {
        ty::TyArray(_, len) => len.unwrap_usize(cx.tcx) as c_longlong,
        _ => -1,
    };
...
pub fn type_metadata<'a, 'tcx>(
    ...
    let MetadataCreationResult {
        ...
        ty::TyRawPtr(ty::TypeAndMut { ty, .. }) | ty::TyRef(_, ty, _) => match ptr_metadata(ty) {
            Ok(res) => res,
...
fn basic_type_metadata<'a, 'tcx>(cx: &CodegenCx<'a, 'tcx>, t: Ty<'tcx>) -> DIType {
    ...
    let (name, encoding) = match t.sty {
        ...
        ty::TyTuple(ref elements) if elements.is_empty() => ("()", DW_ATE_unsigned),
...
fn prepare_enum_metadata<'a, 'tcx>(
    ...
    let enumerators_metadata: Vec<DIDescriptor> = def
...
pub fn create_global_var_metadata(cx: &CodegenCx, def_id: DefId, global: ValueRef) {
