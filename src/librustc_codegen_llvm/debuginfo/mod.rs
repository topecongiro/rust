...
use rustc::ty::{self, InstanceDef, ParamEnv, Ty};
...
pub fn create_function_debug_context<'a, 'tcx>(
    ...
    if let InstanceDef::Item(def_id) = instance.def {
        if cx
            ...
            .flags
            .contains(CodegenFnAttrFlags::NO_DEBUG)
        {
    ...
    fn get_function_signature<'a, 'tcx>(cx: &CodegenCx<'a, 'tcx>, sig: ty::FnSig<'tcx>) -> DIArray {
        ...
        if cx.sess().target.target.options.is_like_msvc {
            ...
            signature.extend(inputs.iter().map(|&t| {
                let t = match t.sty {
                    ty::TyArray(ct, _) if (ct == cx.tcx.types.u8) || cx.layout_of(ct).is_zst() => {
    ...
    fn get_template_parameters<'a, 'tcx>(
        ...
        for (i, actual_type) in substs.types().enumerate() {
            ...
            let actual_type = cx
                .tcx
                .normalize_erasing_regions(ParamEnv::reveal_all(), actual_type);
    ...
    fn get_containing_scope<'cx, 'tcx>(
        cx: &CodegenCx<'cx, 'tcx>,
        instance: Instance<'tcx>,
    ) -> DIScope {
