...
use llvm::{LLVMConstInBoundsGEP, ValueRef};
...
use common::{self, C_null, C_undef, C_usize, CodegenCx};
use consts;
use type_::Type;
...
use super::constant::{const_alloc_to_llvm, scalar_to_llvm};
...
impl<'a, 'tcx> OperandRef<'tcx> {
    pub fn new_zst(cx: &CodegenCx<'a, 'tcx>, layout: TyLayout<'tcx>) -> OperandRef<'tcx> {
    ...
    pub fn from_const(
        ...
        let val = match val {
            ConstValue::Scalar(x) => {
                ...
                let llval = scalar_to_llvm(bx.cx, x, scalar, layout.immediate_llvm_type(bx.cx));
    ...
    pub fn from_immediate_or_packed_pair(
        ...
        let val = if layout.is_llvm_scalar_pair() {
            ...
            OperandValue::Pair(bx.extract_value(llval, 0), bx.extract_value(llval, 1))
        } else {
            OperandValue::Immediate(llval)
        };
    ...
    pub fn extract_field(&self, bx: &Builder<'a, 'tcx>, i: usize) -> OperandRef<'tcx> {
        ...
        let mut val = match (self.val, &self.layout.abi) {
            ...
            (OperandValue::Immediate(llval), &layout::Abi::Vector { .. }) => {
                OperandValue::Immediate(bx.extract_element(llval, C_usize(bx.cx, i as u64)))
            }
