...
impl<'a, 'gcx, 'tcx> CombineFields<'a, 'gcx, 'tcx> {
        ...
        T: Relate<'tcx>,
    {
        ...
        return self.infcx.commit_if_ok(|snapshot| {
            ...
            let (b_prime, skol_map) = self.infcx.skolemize_late_bound_regions(b);
        ...
        U: TypeFoldable<'tcx>,
    {
        ...
        return self.infcx.commit_if_ok(|snapshot| {
            ...
            let ((a_match, a_value), skol_map) = self.infcx.skolemize_late_bound_regions(a_pair);
        ...
        T: Relate<'tcx>,
    {
        ...
        return self.infcx.commit_if_ok(|snapshot| {
            ...
            let result1 = fold_regions_in(self.tcx(), &result0, |r, debruijn| {
                generalize_region(
                    self.infcx, span, snapshot, debruijn, &new_vars, &a_map, &a_vars, &b_vars, r,
                )
            });
        ...
        fn rev_lookup<'a, 'gcx, 'tcx>(
            ...
            for (a_br, a_r) in a_map {
                if *a_r == r {
                    return infcx
                        .tcx
                        .mk_region(ty::ReLateBound(ty::DebruijnIndex::INNERMOST, *a_br));
                }
            }
            ...
            .fold_regions(&value, &mut false, |r, current_depth| {
                match inv_skol_map.get(&r) {
                    ...
                    Some(br) => {
                        ...
                        self.tcx
                            .mk_region(ty::ReLateBound(current_depth.shifted_out(1), br.clone()))
                    }
                }
            });
