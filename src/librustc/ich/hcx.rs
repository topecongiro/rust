...
use rustc_data_structures::stable_hasher::{HashStable, StableHasher, StableHasherResult,
                                           StableHashingContextProvider, ToStableHashKey};
...
impl<'gcx> StableHashingContext<'gcx> {
    ...
    pub fn new(
        ...
        IGNORED_ATTR_NAMES.with(|names| {
            ...
            if names.is_empty() {
                names.extend(ich::IGNORED_ATTRIBUTES.iter().map(|&s| Symbol::intern(s)));
            }
        });
    ...
    pub fn is_ignored_attr(&self, name: Symbol) -> bool {
        IGNORED_ATTR_NAMES.with(|names| names.borrow().contains(&name))
    }

    pub fn hash_hir_item_like<F: FnOnce(&mut Self)>(
        &mut self,
    ...
    pub fn binop_can_panic_at_runtime(&self, binop: hir::BinOp_) -> bool {
        match binop {
            hir::BiAdd | hir::BiSub | hir::BiShl | hir::BiShr | hir::BiMul => {
                self.overflow_checks_enabled
            }

            hir::BiDiv | hir::BiRem => true,
    ...
    R: std_hash::BuildHasher,
{
    ...
    {
        let mut keys: AccumulateVec<[_; 8]> = non_blanket_impls
            .keys()
