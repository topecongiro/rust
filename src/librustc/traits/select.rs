...
impl<'cx, 'gcx, 'tcx> SelectionContext<'cx, 'gcx, 'tcx> {
            ...
            ty::Predicate::Projection(ref data) => {
                ...
                match project::poly_project_and_unify_type(self, &project_obligation) {
                    Ok(Some(subobligations)) => {
                        let result = self
                            .evaluate_predicates_recursively(previous_stack, subobligations.iter());
                        if let Some(key) =
        ...
        match self.is_knowable(stack) {
            ...
            Some(conflict) => {
                ...
                if self.intercrate_ambiguity_causes.is_some() {
                    ...
                    // Heuristics: show the diagnostics when there are no candidates in crate.
                    if let Ok(candidate_set) = self.assemble_candidates(stack) {
                        let no_candidates_apply = candidate_set
                            .vec
                            .iter()
    ...
    fn match_projection_obligation_against_definition_bounds(
        ...
        let poly_trait_predicate = self
            .infcx()
            .resolve_type_vars_if_possible(&obligation.predicate);
        let (skol_trait_predicate, skol_map) = self
            .infcx()
    ...
    fn assemble_candidates_for_unsizing(
        ...
        let may_apply = match (&source.sty, &target.sty) {
            ...
            // (.., T) -> (.., U).
            (&ty::TyTuple(tys_a), &ty::TyTuple(tys_b)) => tys_a.len() == tys_b.len(),
    ...
    fn sized_conditions(
        ...
        match self_ty.sty {
            ...
            ty::TyTuple(tys) => Where(ty::Binder::bind(tys.last().into_iter().cloned().collect())),
    ...
    fn copy_clone_conditions(
        ...
        match self_ty.sty {
            ...
            | ty::TyFnDef(..)
            | ty::TyFnPtr(_)
            | ty::TyError => Where(ty::Binder::dummy(Vec::new())),
            ...
            | ty::TyGeneratorWitness(..)
            | ty::TyForeign(..)
            | ty::TyRef(_, _, hir::MutMutable) => None,
    ...
    fn constituent_types_for_ty(&self, t: Ty<'tcx>) -> Vec<Ty<'tcx>> {
        match t.sty {
            ...
            ty::TyRawPtr(ty::TypeAndMut { ty: element_ty, .. }) | ty::TyRef(_, element_ty, _) => {
                vec![element_ty]
            }
    ...
    fn confirm_candidate(
        ...
        match candidate {
            ...
            ImplCandidate(impl_def_id) => Ok(VtableImpl(
    ...
    > {
        ...
        let (skol_obligation, skol_map) = self
            .infcx()
