...
use super::{
    ConstEvalFailure, FulfillmentError, FulfillmentErrorCode, MismatchedProjectionTypes,
...
impl<'a, 'gcx, 'tcx> InferCtxt<'a, 'gcx, 'tcx> {
    pub fn report_fulfillment_errors(
        ...
        let mut error_map: FxHashMap<_, _> = self
    ...
    fn on_unimplemented_note(
        ...
        if let Some(def) = self_ty.ty_adt_def() {
            ...
            flags.push((
                "_Self".to_string(),
                Some(self.tcx.type_of(def.did).to_string()),
            ));
        }
    ...
    pub fn report_selection_error(
        ...
        let mut err = match *error {
            ...
            ConstEvalFailure(ref err) => match err
                .struct_error(self.tcx.at(span), "could not evaluate constant expression")
            {
                Some(err) => err,
                None => return,
            },
    ...
    pub fn get_fn_like_arguments(&self, node: hir::map::Node) -> (Span, Vec<ArgKind>) {
        match node {
            ...
            hir::map::NodeVariant(&hir::Variant {
                ...
                fields
                    .iter()
                    .map(|field| ArgKind::Arg(format!("{}", field.ident), "_".to_string()))
                    .collect::<Vec<_>>(),
            ),
            hir::map::NodeStructCtor(ref variant_data) => (
                self.tcx
                    .sess
                    .codemap()
                    .def_span(self.tcx.hir.span(variant_data.id())),
                variant_data
                    .fields()
                    .iter()
                    .map(|_| ArgKind::Arg("_".to_owned(), "_".to_owned()))
                    .collect(),
            ),
        ...
        match predicate {
            ...
            ty::Predicate::Subtype(ref data) => {
                if data.references_error() || self.tcx.sess.has_errors() {
                    ...
                    self.need_type_info_err(body_id, obligation.cause.span, a)
                        .emit();
                }
            }
