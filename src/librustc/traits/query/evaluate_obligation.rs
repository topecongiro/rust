...
impl<'cx, 'gcx, 'tcx> InferCtxt<'cx, 'gcx, 'tcx> {
    ...
    pub fn predicate_must_hold(&self, obligation: &PredicateObligation<'tcx>) -> bool {
    ...
    fn evaluate_obligation(&self, obligation: &PredicateObligation<'tcx>) -> EvaluationResult {
        let (c_pred, _) = self.canonicalize_query(&obligation.param_env.and(obligation.predicate));
