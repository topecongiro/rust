...
fn process_predicate<'a, 'gcx, 'tcx>(
    ...
    match obligation.predicate {
        ...
        ty::Predicate::ConstEvaluatable(def_id, substs) => match selcx
            .tcx()
            .lift_to_global(&obligation.param_env)
        {
            None => Ok(None),
            Some(param_env) => match selcx.tcx().lift_to_global(&substs) {
                Some(substs) => {
                    let instance =
                        ty::Instance::resolve(selcx.tcx().global_tcx(), param_env, def_id, substs);
