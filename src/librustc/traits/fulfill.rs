...
use infer::{InferCtxt, InferOk, RegionObligation};
use ty::{self, ToPolyTraitRef, ToPredicate, Ty, TypeFoldable};
use ty::error::ExpectedFound;
use rustc_data_structures::obligation_forest::{Error, ObligationForest};
...
impl<'a, 'gcx, 'tcx> FulfillmentContext<'tcx> {
    ...
    pub fn normalize_projection_type(
        ...
        let normalized =
            project::normalize_projection_type(&mut selcx, param_env, projection_ty, cause, 0);
    ...
    pub fn select_all_or_error(
        ...
        let errors: Vec<_> = self.predicates
...
impl<'a, 'b, 'gcx, 'tcx> ObligationProcessor for FulfillProcessor<'a, 'b, 'gcx, 'tcx> {
    ...
    fn process_obligation(
        ...
        process_predicate(self.selcx, obligation, self.register_region_obligations).map(|os| {
            os.map(|os| {
                os.into_iter()
                    .map(|o| {
    ...
    {
        if self.selcx
            .coinductive_match(cycle.clone().map(|s| s.obligation.predicate))
        {
...
fn trait_ref_type_vars<'a, 'gcx, 'tcx>(
    selcx: &mut SelectionContext<'a, 'gcx, 'tcx>,
...
fn process_predicate<'a, 'gcx, 'tcx>(
    ...
    match obligation.predicate {
        ...
        ty::Predicate::Equate(ref binder) => match selcx.infcx().equality_predicate(
            &obligation.cause,
        ...
        ty::Predicate::RegionOutlives(ref binder) => match selcx
            .infcx()
            .region_outlives_predicate(&obligation.cause, binder)
        {
            Ok(()) => Ok(Some(Vec::new())),
            Err(_) => Err(CodeSelectionError(Unimplemented)),
        },
            ...
            match selcx.tcx().no_late_bound_regions(binder) {
                ...
                None => {
                    ...
                    match selcx.tcx().no_late_bound_regions(&binder) {
                        ...
                        None => Err(CodeSelectionError(Unimplemented)),
        ...
        ty::Predicate::Subtype(ref subtype) => {
            match selcx
                .infcx()
                .subtype_predicate(&obligation.cause, obligation.param_env, subtype)
            {
                None => {
                ...
                Some(Ok(ok)) => Ok(Some(ok.obligations)),
                Some(Err(err)) => {
        ...
        ty::Predicate::ConstEvaluatable(def_id, substs) => {
            match selcx.tcx().lift_to_global(&obligation.param_env) {
                None => Ok(None),
                Some(param_env) => match selcx.tcx().lift_to_global(&substs) {
                    None => {
                        pending_obligation.stalled_on = substs.types().collect();
                        Ok(None)
                    }
                    Some(substs) => match selcx
                        .tcx()
                        .at(obligation.cause.span)
