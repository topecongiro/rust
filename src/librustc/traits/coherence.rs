...
fn uncovered_tys<'tcx>(tcx: TyCtxt, ty: Ty<'tcx>, infer_is_local: InferIsLocal) -> Vec<Ty<'tcx>> {
...
fn ty_is_local(tcx: TyCtxt, ty: Ty, infer_is_local: InferIsLocal) -> bool {
    ty_is_local_constructor(ty, infer_is_local)
        || fundamental_ty(tcx, ty)
            && ty.walk_shallow()
                .any(|t| ty_is_local(tcx, t, infer_is_local))
}
...
fn ty_is_local_constructor(ty: Ty, infer_is_local: InferIsLocal) -> bool {
    ...
    match ty.sty {
        ...
        ty::TyInfer(..) => infer_is_local.0,

        ty::TyAdt(def, _) => def.did.is_local(),

        ty::TyForeign(did) => did.is_local(),

        ty::TyDynamic(ref tt, ..) => tt.principal().map_or(false, |p| p.def_id().is_local()),
