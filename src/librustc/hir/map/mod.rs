...
pub use self::definitions::{DefKey, DefPath, DefPathData, DefPathHash, Definitions,
    ...
    fn suffix_matches(&self, parent: NodeId) -> bool {
        ...
        fn find_first_mod_parent<'a>(map: &'a Map, mut id: NodeId) -> Option<(NodeId, Name)> {
            loop {
                match map.find(id) {
                    None => return None,
                    Some(NodeItem(item)) if item_is_mod(&item) => return Some((id, item.name)),
                    _ => {}
                }
...
fn node_id_to_string(map: &Map, id: NodeId, include_id: bool) -> String {
    ...
    match map.find(id) {
        ...
        Some(NodeField(ref field)) => format!("field {} in {}{}", field.name, path_str(), id_str),
