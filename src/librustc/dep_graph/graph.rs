...
impl DepGraph {
    ...
    pub fn fingerprint_of(&self, dep_node: &DepNode) -> Fingerprint {
        match self.fingerprints.borrow().get(dep_node) {
            ...
            None => bug!("Could not find current fingerprint for {:?}", dep_node),
        }
    }
    ...
    pub fn serialize(&self) -> SerializedDepGraph {
        ...
        let nodes: IndexVec<_, _> = current_dep_graph
            .nodes
            .iter()
            .map(|dep_node| {
    ...
    pub fn try_mark_green(&self, tcx: TyCtxt, dep_node: &DepNode) -> Option<DepNodeIndex> {
