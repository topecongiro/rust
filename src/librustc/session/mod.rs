...
use session::config::{BorrowckMode, DebugInfoLevel, Epoch, OutputType};
use ty::tls;
...
impl Session {
    ...
    pub fn emit_end_regions(&self) -> bool {
        ...
        self.opts.debugging_opts.emit_end_regions || self.opts.debugging_opts.mir_emit_validate > 0
            || self.use_mir()
    }
...
pub fn build_session_with_codemap(
    ...
    let emitter: Box<Emitter> =
        match (sopts.error_format, emitter_dest) {
            ...
            (config::ErrorOutputType::Json(pretty), None) => Box::new(JsonEmitter::stderr(
                Some(registry),
                codemap.clone(),
                pretty,
