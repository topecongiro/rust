...
impl<'a, 'tcx, O: DataFlowOperator> pprust::PpAnn for DataFlowContext<'a, 'tcx, O> {
    ...
    fn pre(&self, ps: &mut pprust::State, node: pprust::AnnNode) -> io::Result<()> {
        let id = match node {
            pprust::NodeName(_) => return Ok(()),
            pprust::NodeExpr(expr) => expr.hir_id.local_id,
            pprust::NodeBlock(blk) => blk.hir_id.local_id,
            pprust::NodeItem(_) | pprust::NodeSubItem(_) => return Ok(()),
...
fn build_local_id_to_index(
    ...
    fn add_entries_from_fn_body(
        index: &mut FxHashMap<hir::ItemLocalId, Vec<CFGIndex>>,
        body: &hir::Body,
        entry: CFGIndex,
    ) {
...
impl<'a, 'tcx, O: DataFlowOperator> DataFlowContext<'a, 'tcx, O> {
    pub fn new(
        ...
        debug!(
            "DataFlowContext::new(analysis_name: {}, id_range={:?}, \
             ...
             num_nodes: {}",
            analysis_name, id_range, bits_per_id, words_per_id, num_nodes
        );
    ...
    pub fn add_gen(&mut self, id: hir::ItemLocalId, bit: usize) {
        ...
        debug!("{} add_gen(id={:?}, bit={})", self.analysis_name, id, bit);
    ...
    pub fn add_kill(&mut self, kind: KillFrom, id: hir::ItemLocalId, bit: usize) {
        ...
        debug!("{} add_kill(id={:?}, bit={})", self.analysis_name, id, bit);
        assert!(self.local_id_to_index.contains_key(&id));
        assert!(self.bits_per_id > 0);
    ...
    pub fn each_bit_on_entry<F>(&self, id: hir::ItemLocalId, mut f: F) -> bool
    where
        F: FnMut(usize) -> bool,
    {
    ...
    pub fn each_bit_for_node<F>(&self, e: EntryOrExit, cfgidx: CFGIndex, f: F) -> bool
    where
        F: FnMut(usize) -> bool,
    {
    ...
    pub fn each_gen_bit<F>(&self, id: hir::ItemLocalId, mut f: F) -> bool
    where
        F: FnMut(usize) -> bool,
    {
    ...
    pub fn add_kills_from_flow_exits(&mut self, cfg: &cfg::CFG) {
        ...
        cfg.graph.each_edge(|_edge_index, edge| {
            ...
            for &id in &edge.data.exiting_scopes {
                ...
                match opt_cfg_idx {
                    Some(indices) => for &cfg_idx in indices {
...
impl<'a, 'b, 'tcx, O: DataFlowOperator> PropagationContext<'a, 'b, 'tcx, O> {
    fn walk_cfg(&mut self, cfg: &cfg::CFG, nodes_po: &[CFGIndex], in_out: &mut [usize]) {
        debug!(
            "DataFlowContext::walk_cfg(in_out={}) {}",
            bits_to_string(in_out),
            self.dfcx.analysis_name
        );
    ...
    fn reset(&mut self, bits: &mut [usize]) {
        let e = if self.dfcx.oper.initial_value() {
            usize::MAX
        } else {
            0
        };
    ...
    fn propagate_bits_into_entry_set_for(&mut self, pred_bits: &[usize], edge: &cfg::CFGEdge) {
