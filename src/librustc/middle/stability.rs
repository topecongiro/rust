...
use syntax::feature_gate::{emit_feature_err, find_lang_feature_accepted_version, GateIssue};
use syntax::attr::{self, Deprecation, Stability};
use util::nodemap::{FxHashMap, FxHashSet};

use hir;
use hir::{Generics, HirId, Item, StructField, Variant};
...
impl<'a, 'tcx: 'a> Annotator<'a, 'tcx> {
        ...
        F: FnOnce(&mut Self),
    {
        if self.tcx.sess.features.borrow().staged_api {
            ...
            if let Some(mut stab) = attr::find_stability(self.tcx.sess.diagnostic(), attrs, item_sp)
            {
                        ...
                        && stab.rustc_depr.is_none())
                {
                    self.tcx
                        .sess
                        .span_err(item_sp, "This stability annotation is useless");
                }
            ...
            if let Some(depr) = attr::find_deprecation(self.tcx.sess.diagnostic(), attrs, item_sp) {
                if kind == AnnotationKind::Prohibited {
                    self.tcx
                        .sess
                        .span_err(item_sp, "This deprecation annotation is useless");
                }
...
impl<'a, 'tcx: 'a> MissingStabilityAnnotations<'a, 'tcx> {
    fn check_missing_stability(&self, id: NodeId, span: Span) {
        ...
        let is_error =
            !self.tcx.sess.opts.test && stab.is_none() && self.access_levels.is_reachable(id);
        if is_error {
            self.tcx
                .sess
                .span_err(span, "This node does not have a stability attribute");
        }
    }
}
...
impl<'a, 'tcx> Index<'tcx> {
    pub fn new(tcx: TyCtxt<'a, 'tcx, 'tcx>) -> Index<'tcx> {
        let is_staged_api = tcx.sess.opts.debugging_opts.force_unstable_if_unmarked
...
impl<'a, 'gcx, 'tcx> TyCtxt<'a, 'gcx, 'tcx> {
    ...
    fn skip_stability_check_due_to_privacy(self, mut def_id: DefId) -> bool {
        ...
        match self.describe_def(def_id) {
            Some(Def::Method(_)) | Some(Def::AssociatedTy(_)) | Some(Def::AssociatedConst(_)) => {
    ...
    pub fn check_stability(self, def_id: DefId, id: NodeId, span: Span) {
        if span.allows_unstable() {
            debug!(
                "stability: \
                 skipping span={:?} since it is internal",
                span
            );
            return;
        }
        ...
        if let Some(&Stability {
            rustc_depr: Some(attr::RustcDeprecation { reason, .. }),
            ..
        }) = stability
        {
        ...
        match stability {
            Some(&Stability {
                level: attr::Unstable { ref reason, issue },
                ref feature,
                ..
            }) => {
                ...
                let error_id = (
                    DiagnosticMessageId::StabilityId(issue),
                    span_key,
                    msg.clone(),
                );
...
impl<'a, 'tcx> Visitor<'tcx> for Checker<'a, 'tcx> {
        ...
        match item.node {
                ...
                if let Def::Trait(trait_did) = t.path.def {
                    for impl_item_ref in impl_item_refs {
                        ...
                        let trait_item_def_id = self.tcx
                            ...
                            .find(|item| item.name == impl_item.name)
                            .map(|item| item.def_id);
                        if let Some(def_id) = trait_item_def_id {
                            // Pass `DUMMY_NODE_ID` to skip deprecation warnings.
                            self.tcx
                                .check_stability(def_id, ast::DUMMY_NODE_ID, impl_item.span);
                        }
                    }
                }
            }
            ...
            hir::ItemUnion(..) if !self.tcx.sess.features.borrow().untagged_unions => {
                ...
                if adt_def.has_dtor(self.tcx) {
                    ...
                    if !param_env
                        .can_type_implement_copy(self.tcx, ty, item.span)
                        .is_ok()
                    {
                        emit_feature_err(
                            &self.tcx.sess.parse_sess,
    ...
    fn visit_path(&mut self, path: &'tcx hir::Path, id: ast::NodeId) {
        match path.def {
            Def::Local(..) | Def::Upvar(..) | Def::PrimTy(..) | Def::SelfTy(..) | Def::Err => {}
