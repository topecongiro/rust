...
use ty::{self, TyCtxt};
use util::nodemap::{FxHashSet, NodeSet};
...
fn item_might_be_inlined(item: &hir::Item, attrs: CodegenFnAttrs) -> bool {
    ...
    match item.node {
        hir::ItemImpl(_, _, _, ref generics, ..) | hir::ItemFn(.., ref generics, _) => {
...
fn method_might_be_inlined<'a, 'tcx>(
    tcx: TyCtxt<'a, 'tcx, 'tcx>,
    impl_item: &hir::ImplItem,
    impl_src: DefId,
) -> bool {
    let codegen_fn_attrs = tcx.codegen_fn_attrs(impl_item.hir_id.owner_def_id());
    if codegen_fn_attrs.requests_inline() || generics_require_inlining(&impl_item.generics) {
        return true;
    }
    if let Some(impl_node_id) = tcx.hir.as_local_node_id(impl_src) {
        match tcx.hir.find(impl_node_id) {
            Some(hir_map::NodeItem(item)) => item_might_be_inlined(&item, codegen_fn_attrs),
...
impl<'a, 'tcx> Visitor<'tcx> for ReachableContext<'a, 'tcx> {
    ...
    fn visit_expr(&mut self, expr: &'tcx hir::Expr) {
        let def = match expr.node {
            hir::ExprPath(ref qpath) => Some(self.tables.qpath_def(qpath, expr.hir_id)),
            hir::ExprMethodCall(..) => Some(self.tables.type_dependent_defs()[expr.hir_id]),
            _ => None,
        };
...
impl<'a, 'tcx> ReachableContext<'a, 'tcx> {
    ...
    fn def_id_represents_local_inlined_item(&self, def_id: DefId) -> bool {
        ...
        match self.tcx.hir.find(node_id) {
            Some(hir_map::NodeItem(item)) => match item.node {
                hir::ItemFn(..) => item_might_be_inlined(&item, self.tcx.codegen_fn_attrs(def_id)),
                _ => false,
            },
            Some(hir_map::NodeTraitItem(trait_method)) => match trait_method.node {
            ...
            Some(hir_map::NodeImplItem(impl_item)) => {
                match impl_item.node {
                    ...
                    hir::ImplItemKind::Method(..) => {
                        ...
                        if generics_require_inlining(&impl_item.generics) || attrs.requests_inline()
                        {
                            true
                        } else {
    ...
    fn propagate_node(&mut self, node: &hir_map::Node<'tcx>, search_item: ast::NodeId) {
        if !self.any_library {
...
fn reachable_set<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, crate_num: CrateNum) -> ReachableSet {
    ...
    }
    {
        ...
        tcx.hir
            .krate()
            .visit_all_item_likes(&mut collect_private_impl_items);
    }
