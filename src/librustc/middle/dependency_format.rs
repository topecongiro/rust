...
use middle::cstore::LinkagePreference::{self, RequireDynamic, RequireStatic};
...
fn calculate_type<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, ty: config::CrateType) -> DependencyList {
    let sess = &tcx.sess;
    ...
    let preferred_linkage = match ty {
        ...
        config::CrateTypeExecutable if !sess.opts.cg.prefer_dynamic || sess.crt_static() => {
            Linkage::Static
        }
    ...
    activate_injected_dep(sess.injected_panic_runtime.get(), &mut ret, &|cnum| {
...
fn activate_injected_allocator(sess: &session::Session, list: &mut DependencyList) {
