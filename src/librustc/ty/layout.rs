...
impl<'a, 'tcx> LayoutCx<'tcx, TyCtxt<'a, 'tcx, 'tcx>> {
                ...
                if zst_offsets.all(|(_, o)| o.bytes() == 0) {
                    ...
                    match (
                        ...
                        (Some((i, field)), None, None) => {
                            ...
                            if offsets[i].bytes() == 0 && align.abi() == field.align.abi()
                                && size == field.size
                            {
                                match field.abi {
                ...
                if !def.repr.inhibit_enum_layout_opt() && no_explicit_discriminants {
                    ...
                    if let Some(i) = dataful_variant {
                        ...
                        for (field_index, field) in variants[i].iter().enumerate() {
                            let (offset, niche, niche_start) = match field.find_niche(self, count)?
                            {
                                Some(niche) => niche,
                                None => continue,
                            };
    ...
    fn record_layout_for_printing(self, layout: TyLayout<'tcx>) {
        ...
        if !self.tcx.sess.opts.debugging_opts.print_type_sizes || layout.ty.has_param_types()
            || layout.ty.has_self_ty() || !self.param_env.caller_bounds.is_empty()
        {
            return;
        }
    ...
    fn record_layout_for_printing_outlined(self, layout: TyLayout<'tcx>) {
        ...
        let record = |kind, opt_discr_size, variants| {
            ...
            self.tcx.sess.code_stats.borrow_mut().record_type_size(
                kind,
                type_desc,
                layout.align,
                layout.size,
...
impl<'a, 'tcx> TyLayout<'tcx> {
        ...
        C::TyLayout: MaybeResult<TyLayout<'tcx>>,
    {
        ...
        cx.layout_of(match self.ty.sty {
            ...
            | ty::TyForeign(..)
            | ty::TyDynamic(..) => bug!("TyLayout::field_type({:?}): not applicable", self),
