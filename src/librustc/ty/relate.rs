...
impl<'tcx> Relate<'tcx> for Vec<ty::PolyExistentialProjection<'tcx>> {
        ...
        'tcx: 'a,
    {
        ...
        // same length.
        if a.len() != b.len() {
            Err(TypeError::ProjectionBoundsLength(expected_found(
                relation,
                &a.len(),
                &b.len(),
            )))
...
impl<'tcx> Relate<'tcx> for ty::TraitRef<'tcx> {
        ...
        'tcx: 'a,
    {
        ...
        if a.def_id != b.def_id {
            Err(TypeError::Traits(expected_found(
                relation, &a.def_id, &b.def_id,
            )))
        } else {
    ...
    'tcx: 'a,
{
    ...
    match (a_sty, b_sty) {
        ...
        (&ty::TyError, _) | (_, &ty::TyError) => Ok(tcx.types.err),
        ...
        (&ty::TyParam(ref a_p), &ty::TyParam(ref b_p)) if a_p.idx == b_p.idx => Ok(a),

        (&ty::TyAdt(a_def, a_substs), &ty::TyAdt(b_def, b_substs)) if a_def == b_def => {
            let substs = relation.relate_item_substs(a_def.did, a_substs, b_substs)?;
            Ok(tcx.mk_adt(a_def, substs))
        }

        (&ty::TyForeign(a_id), &ty::TyForeign(b_id)) if a_id == b_id => Ok(tcx.mk_foreign(a_id)),
        ...
        (&ty::TyGenerator(a_id, a_substs, movability), &ty::TyGenerator(b_id, b_substs, _))
            if a_id == b_id =>
        {
        ...
        (&ty::TyGeneratorWitness(a_types), &ty::TyGeneratorWitness(b_types)) => {
            ...
            let types = relation
                .relate(&a_types, &b_types)?
                .map_bound(|witness| witness.0);
            Ok(tcx.mk_generator_witness(types))
        }

        (&ty::TyClosure(a_id, a_substs), &ty::TyClosure(b_id, b_substs)) if a_id == b_id => {
        ...
        (&ty::TyRef(a_r, a_ty, a_mutbl), &ty::TyRef(b_r, b_ty, b_mutbl)) => {
            ...
            let a_mt = ty::TypeAndMut {
                ty: a_ty,
                mutbl: a_mutbl,
            };
            let b_mt = ty::TypeAndMut {
                ty: b_ty,
                mutbl: b_mutbl,
            };
            let mt = relation.relate(&a_mt, &b_mt)?;
            ...
            match (to_u64(sz_a), to_u64(sz_b)) {
                (Ok(sz_a_u64), Ok(sz_b_u64)) => {
                    if sz_a_u64 == sz_b_u64 {
                        ...
                        Err(TypeError::FixedArraySize(expected_found(
                            relation, &sz_a_u64, &sz_b_u64,
                        )))
                    }
                }
                // We reported an error or will ICE, so we can return TyError.
                (Err(ErrorReported), _) | (_, Err(ErrorReported)) => Ok(tcx.types.err),
            }
        }
        ...
        (&ty::TyTuple(as_), &ty::TyTuple(bs)) => {
            if as_.len() == bs.len() {
                ...
                Err(TypeError::TupleSize(expected_found(
                    relation,
                    &as_.len(),
                    &bs.len(),
                )))
...
impl<'tcx> Relate<'tcx> for &'tcx ty::Slice<ty::ExistentialPredicate<'tcx>> {
    ...
    {
        if a.len() != b.len() {
            return Err(TypeError::ExistentialMismatch(expected_found(
                relation, a, b,
            )));
        }
        ...
        let v = a.iter().zip(b.iter()).map(|(ep_a, ep_b)| {
            ...
            match (*ep_a, *ep_b) {
                ...
                _ => Err(TypeError::ExistentialMismatch(expected_found(
                    relation, a, b,
                ))),
            }
        });
