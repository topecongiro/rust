...
use hir;
use hir::def_id::{CrateNum, DefId, DefIndex, LocalDefId, LOCAL_CRATE, RESERVED_FOR_INCR_COMP_CACHE};
use hir::map::definitions::DefPathHash;
use middle::cstore::CrateStore;
use rustc_data_structures::fx::FxHashMap;
use rustc_data_structures::indexed_vec::{Idx, IndexVec};
use rustc_serialize::{opaque, Decodable, Decoder, Encodable, Encoder, SpecializedDecoder,
                      SpecializedEncoder, UseSpecializedDecodable, UseSpecializedEncodable};
...
impl<'sess> OnDiskCache<'sess> {
        ...
        let (prev_diagnostics, query_result_index) = {
            ...
            let prev_diagnostics: FxHashMap<_, _> = {
                let diagnostics: EncodedPrevDiagnostics =
                    decode_tagged(&mut decoder, PREV_DIAGNOSTICS_TAG).expect(
                        "Error while trying to decode previous session \
            ...
            let query_result_index_pos = {
                ...
                decoder
                    .with_position(pos_pos, |decoder| IntEncodedWithFixedSize::decode(decoder))
                    .expect("Error while trying to decode query result index position.")
                    .0 as usize
            };

            // Decode the query result index itself
            let query_result_index: EncodedQueryResultIndex = decoder
                .with_position(query_result_index_pos, |decoder| {
        ...
        E: ty_codec::TyEncoder,
    {
        ...
        let prev_filemap_starts: BTreeMap<_, _> = self.codemap
            .files()
            .iter()
        ...
        // Encode Diagnostics
        let diagnostics: EncodedPrevDiagnostics = self.current_diagnostics
            .borrow()
            .iter()
    ...
    pub fn load_diagnostics(&self, dep_node_index: SerializedDepNodeIndex) -> Vec<Diagnostic> {
        self.prev_diagnostics
            .get(&dep_node_index)
            .cloned()
            .unwrap_or(vec![])
    }
    ...
    pub fn store_diagnostics(&self, dep_node_index: DepNodeIndex, diagnostics: Vec<Diagnostic>) {
        let mut current_diagnostics = self.current_diagnostics.borrow_mut();
        let prev = current_diagnostics.insert(dep_node_index, diagnostics);
        debug_assert!(prev.is_none());
    }

    pub fn load_query_result<'a, 'tcx, T>(
        &self,
        tcx: TyCtxt<'a, 'tcx, 'tcx>,
        ...
        T: Decodable,
    {
        ...
        match decode_tagged(&mut decoder, dep_node_index) {
            Ok(value) => value,
            Err(e) => bug!("Could not decode cached query result: {}", e),
        }
    }
    ...
    pub fn store_diagnostics_for_anon_node(
        ...
        let x = current_diagnostics
            .entry(dep_node_index)
            .or_insert_with(|| mem::replace(&mut diagnostics, Vec::new()));
    ...
    fn compute_cnum_map(
        ...
        let current_cnums = tcx.all_crate_nums(LOCAL_CRATE)
            .iter()
            .map(|&cnum| {
                let crate_name = tcx.original_crate_name(cnum).as_str().to_string();
                let crate_disambiguator = tcx.crate_disambiguator(cnum);
...
impl<'a, 'tcx, 'x> CacheDecoder<'a, 'tcx, 'x> {
    fn find_filemap_prev_bytepos(
        &self,
        prev_bytepos: BytePos,
    ) -> Option<(BytePos, StableFilemapId)> {
        for (start, id) in self.prev_filemap_starts
            .range(BytePos(0)..=prev_bytepos)
            .rev()
        {
...
impl<'a, 'tcx: 'a, 'x> ty_codec::TyDecoder<'a, 'tcx> for CacheDecoder<'a, 'tcx, 'x> {
    ...
    where
        F: FnOnce(&mut Self) -> Result<ty::Ty<'tcx>, Self::Error>,
    {
        let tcx = self.tcx();
    ...
    E: 'enc + ty_codec::TyEncoder,
{
    ...
    fn specialized_encode(
        ...
        ty_codec::encode_predicates(self, predicates, |encoder| {
...
impl<'a, 'tcx, 'x> SpecializedDecoder<IntEncodedWithFixedSize> for CacheDecoder<'a, 'tcx, 'x> {
...
fn encode_query_results<'enc, 'a, 'tcx, Q, E>(
    tcx: TyCtxt<'a, 'tcx, 'tcx>,
    encoder: &mut CacheEncoder<'enc, 'a, 'tcx, E>,
    query_result_index: &mut EncodedQueryResultIndex,
) -> Result<(), E::Error>
where
    Q: super::plumbing::GetCacheInternal<'tcx>,
    E: 'enc + TyEncoder,
    Q::Value: Encodable,
{
