...
use syntax::print::pprust::{
    bounds_to_string, generic_params_to_string, path_to_string, ty_to_string,
};
use syntax::ptr::P;
...
use rls_data::{
    CratePreludeData, Def, DefKind, GlobalCrateId, Import, ImportKind, Ref, RefKind, Relation,
...
impl<'l, 'tcx: 'l, 'll, O: DumpOutput + 'll> DumpVisitor<'l, 'tcx, 'll, O> {
    ...
    fn process_method(
        ...
        if let Some(mut method_data) = self.save_ctxt.get_method_data(id, name.name, span) {
            ...
            self.dumper
                .dump_def(&access_from!(self.save_ctxt, vis, id), method_data);
        }
    ...
    fn process_use_tree(
        ...
        match use_tree.kind {
            ast::UseTreeKind::Simple(..) => {
                ...
                let alias_span = self
                    .span
                    .sub_span_after_keyword(use_tree.span, keywords::As);
            ...
            ast::UseTreeKind::Glob => {
                ...
                let sub_span = self
                    .span
                    .sub_span_of_token(use_tree.span, token::BinOp(token::Star));
                if !self.span.filter_generated(sub_span, use_tree.span) {
                    let span = self.span_from_span(sub_span.expect("No span found for use glob"));
                    self.dumper.import(
                        &access,
                        Import {
