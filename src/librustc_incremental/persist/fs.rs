...
pub fn lock_file_path(session_dir: &Path) -> PathBuf {
    ...
    let dash_indices: Vec<_> = directory_name
        .match_indices("-")
        .map(|(idx, _)| idx)
        .collect();
    ...
    crate_dir
        .join(&directory_name[0..dash_indices[2]])
...
pub fn prepare_session_directory(
    ...
    loop {
        ...
        debug!(
            "attempting to copy data from source: {}",
...
pub fn finalize_session_directory(sess: &Session, svh: Svh) {
    ...
    debug!(
        "finalize_session_directory() - session directory: {}",
        incr_comp_session_dir.display()
    );
    ...
    let dash_indices: Vec<_> = old_sub_dir_name
        .match_indices("-")
        .map(|(idx, _)| idx)
        .collect();
    if dash_indices.len() != 3 {
    ...
    let new_path = incr_comp_session_dir
        .parent()
        .unwrap()
        .join(new_sub_dir_name);
    debug!(
        "finalize_session_directory() - new path: {}",
        new_path.display()
    );
        ...
        match entry {
            Ok(entry) => {
                ...
                match fs_util::link_or_copy(source_path, target_file_path) {
                    Ok(fs_util::LinkOrCopy::Link) => files_linked += 1,
                    Ok(fs_util::LinkOrCopy::Copy) => files_copied += 1,
                    Err(_) => return Err(()),
                }
            }
            Err(_) => return Err(()),
...
fn generate_session_dir_path(crate_dir: &Path) -> PathBuf {
    ...
    debug!(
        "generate_session_dir_path: random_number = {}",
        random_number
    );
    ...
    let directory_name = format!(
        "s-{}-{}-working",
        timestamp,
        base_n::encode(random_number as u64, INT_ENCODE_BASE)
    );
    debug!(
        "generate_session_dir_path: directory_name = {}",
        directory_name
    );
    let directory_path = crate_dir.join(directory_name);
    debug!(
        "generate_session_dir_path: directory_path = {}",
    ...
    I: Iterator<Item = PathBuf>,
{
    ...
    for session_dir in iter {
        debug!(
            "find_source_directory_in_iter - inspecting `{}`",
            session_dir.display()
        );
        ...
        if source_directories_already_tried.contains(&session_dir)
            || !is_session_directory(&directory_name) || !is_finalized(&directory_name)
        {
        ...
        let timestamp = extract_timestamp_from_session_dir(&directory_name).unwrap_or_else(|_| {
            bug!(
                "unexpected incr-comp session dir: {}",
                session_dir.display()
            )
        });
...
pub fn garbage_collect_session_directories(sess: &Session) -> io::Result<()> {
    ...
    for (lock_file_name, directory_name) in &lock_file_to_session_dir {
        debug!(
            "garbage_collect_session_directories() - inspecting: {}",
            directory_name
        );
...
fn all_except_most_recent(
    ...
    let most_recent = deletion_candidates
        .iter()
...
fn test_find_source_directory_in_iter() {
    ...
    // Handle empty
    assert_eq!(
        find_source_directory_in_iter(vec![].into_iter(), &already_visited),
        None
    );
