...
fn expand_deriving_encodable_imp(
    cx: &mut ExtCtxt,
    span: Span,
    mitem: &MetaItem,
    item: &Annotatable,
    ...
    let trait_def = TraitDef {
        ...
        methods: vec![
            MethodDef {
                ...
                generics: LifetimeBounds {
                    ...
                    bounds: vec![
                        (
                            typaram,
                            vec![
                                Path::new_(vec![krate, "Encoder"], None, vec![], PathKind::Global),
                            ],
                        ),
                    ],
                },
                ...
                args: vec![
                    Ptr(
                        Box::new(Literal(Path::new_local(typaram))),
    ...
    return match *substr.fields {
        Struct(_, ref fields) => {
            ...
            ) in fields.iter().enumerate()
            {
                ...
                let expr = if attrs
                    .iter()
                    .any(|a| a.check_name("rustc_serialize_exclude_null"))
                {
