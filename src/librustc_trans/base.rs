...
use common::{self, val_ty, C_array, C_struct_in_context};
use consts;
...
use monomorphize::partitioning::{self, CodegenUnit, CodegenUnitExt, PartitioningStrategy};
use rustc_trans_utils::symbol_names_test;
use time_graph;
use trans_item::{BaseMonoItemExt, DefPathBasedNames, MonoItem, MonoItemExt};
use type_::Type;
...
pub fn call_memcpy(bx: &Builder, dst: ValueRef, src: ValueRef, n_bytes: ValueRef, align: Align) {
    let cx = bx.cx;
...
pub fn trans_instance<'a, 'tcx>(cx: &CodegenCx<'a, 'tcx>, instance: Instance<'tcx>) {
    ...
    if !cx.sess().no_landing_pads() || cx.sess().target.target.options.is_like_windows {
...
pub fn set_link_section(cx: &CodegenCx, llval: ValueRef, attrs: &[ast::Attribute]) {
...
fn maybe_create_entry_wrapper(cx: &CodegenCx) {
    let (main_def_id, span) = match *cx.sess().entry_fn.borrow() {
        Some((id, span)) => (cx.tcx.hir.local_def_id(id), span),
        None => return,
    };
    ...
    fn create_entry_fn<'cx>(
        ...
        let (start_fn, args) = if use_start_lang_item {
            ...
            let start_fn = callee::resolve_and_get_fn(
                cx,
                start_def_id,
                cx.tcx.mk_substs(iter::once(Kind::from(main_ret_ty))),
            );
            (
                start_fn,
                vec![
                    bx.pointercast(rust_main, Type::i8p(cx).ptr_to()),
                    arg_argc,
                    arg_argv,
                ],
            )
...
fn write_metadata<'a, 'gcx>(
    ...
    let llglobal =
        unsafe { llvm::LLVMAddGlobal(metadata_llmod, val_ty(llconst).to_ref(), buf.as_ptr()) };
    unsafe {
...
fn compile_codegen_unit<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, cgu: InternedString) -> Stats {
    ...
    fn module_translation<'a, 'tcx>(
        ...
        let module = {
            let trans_items = cx.codegen_unit.items_in_deterministic_order(cx.tcx);
            ...
            if !cx.used_statics.borrow().is_empty() {
                ...
                unsafe {
                    let g = llvm::LLVMAddGlobal(cx.llmod, val_ty(array).to_ref(), name.as_ptr());
