...
pub trait TransItemExt<'a, 'tcx>: fmt::Debug + BaseTransItemExt<'a, 'tcx> {
    ...
    fn predefine(&self, ccx: &CrateContext<'a, 'tcx>, linkage: Linkage, visibility: Visibility) {
        debug!(
    ...
    fn local_span(&self, tcx: TyCtxt<'a, 'tcx, 'tcx>) -> Option<Span> {
        match *self.as_trans_item() {
            TransItem::Fn(Instance { def, .. }) => tcx.hir.as_local_node_id(def.def_id()),
    ...
    fn is_generic_fn(&self) -> bool {
        match *self.as_trans_item() {
            TransItem::Fn(ref instance) => instance.substs.types().next().is_some(),
            TransItem::Static(..) | TransItem::GlobalAsm(..) => false,
        }
    }

    fn to_raw_string(&self) -> String {
        match *self.as_trans_item() {
            TransItem::Fn(instance) => format!(
                "Fn({:?}, {})",
                instance.def,
                instance.substs.as_ptr() as usize
            ),
            TransItem::Static(id) => format!("Static({:?})", id),
            TransItem::GlobalAsm(id) => format!("GlobalAsm({:?})", id),
        }
    }
}
