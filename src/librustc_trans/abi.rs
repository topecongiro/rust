...
impl CastTarget {
    pub fn size(&self, cx: &CodegenCx) -> Size {
        match *self {
            ...
            CastTarget::Pair(a, b) => {
                (a.size.abi_align(a.align(cx)) + b.size).abi_align(self.align(cx))
            }
        }
    }
    ...
    pub fn align(&self, cx: &CodegenCx) -> Align {
        match *self {
            ...
            CastTarget::Pair(a, b) => cx.data_layout()
                .aggregate_align
                .max(a.align(cx))
                .max(b.align(cx)),
        }
    ...
    pub fn llvm_type(&self, cx: &CodegenCx) -> Type {
        match *self {
            ...
            CastTarget::Pair(a, b) => Type::struct_(cx, &[a.llvm_type(cx), b.llvm_type(cx)], false),
        }
    }
}
...
impl<'a, 'tcx> FnType<'tcx> {
    pub fn of_instance(cx: &CodegenCx<'a, 'tcx>, instance: &ty::Instance<'tcx>) -> Self {
