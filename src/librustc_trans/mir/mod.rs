...
pub fn trans_mir<'a, 'tcx: 'a>(
    ...
    debug!("fn_ty: {:?}", fn_ty);
    let debug_context = debuginfo::create_function_debug_context(ccx, instance, sig, llfn, mir);
    ...
    let block_bcxs: IndexVec<mir::BasicBlock, BasicBlockRef> = mir.basic_blocks()
        .indices()
        .map(|bb| {
        ...
        let mut allocate_local = |local| {
            ...
            if let Some(name) = decl.name {
                ...
                if local == mir::RETURN_POINTER && mircx.fn_ty.ret.is_indirect() {
                    ...
                    LocalRef::Lvalue(LvalueRef::new_sized(
                        llretptr,
                        layout,
...
fn create_funclets<'a, 'tcx>(
    ...
    block_bcxs
        .iter_enumerated()
        .zip(cleanup_kinds)
        .map(|((bb, &llbb), cleanup_kind)| match *cleanup_kind {
        ...
        .map(|(arg_index, local)| {
            ...
            arg_scope.map(|scope| {
                ...
                let upvar_tys = match closure_layout.ty.sty {
                    ty::TyClosure(def_id, substs) | ty::TyGenerator(def_id, substs, _) => {
