...
use common::{C_bool, C_i32, C_null, C_u32, C_u64, C_u8, C_uint, C_uint_big, C_usize};
use consts;
use monomorphize;
use type_::Type;
...
impl<'a, 'tcx> MirContext<'a, 'tcx> {
        ...
        match *rvalue {
            ...
            mir::Rvalue::Repeat(ref elem, count) => {
                ...
                if let OperandValue::Immediate(v) = tr_elem.val {
                    let align = dest.alignment.non_abi().unwrap_or(tr_elem.layout.align);
            ...
            mir::Rvalue::Aggregate(ref kind, ref operands) => {
                let (dest, active_field_index) = match **kind {
                    mir::AggregateKind::Adt(adt_def, variant_index, _, active_field_index) => {
                        ...
                        if adt_def.is_enum() {
                            (
                                dest.project_downcast(&bcx, variant_index),
                                active_field_index,
                            )
                        } else {
    ...
    pub fn trans_rvalue_operand(
        ...
        match *rvalue {
            mir::Rvalue::Cast(ref kind, ref source, mir_cast_ty) => {
                ...
                let val = match *kind {
                    mir::CastKind::ReifyFnPointer => match operand.layout.ty.sty {
                        ty::TyFnDef(def_id, substs) => OperandValue::Immediate(
                            callee::resolve_and_get_fn(bcx.ccx, def_id, substs),
                        ),
                        _ => bug!("{} cannot be reified to a fn ptr", operand.layout.ty),
                    },
                    mir::CastKind::ClosureFnPointer => match operand.layout.ty.sty {
                    ...
                    mir::CastKind::Unsize => {
                        ...
                        match operand.val {
                            ...
                            OperandValue::Immediate(lldata) => {
                                // "standard" unsize
                                let (lldata, llextra) =
                                    base::unsize_thin_ptr(&bcx, lldata, operand.layout.ty, cast.ty);
                                OperandValue::Pair(lldata, llextra)
                            }
                            OperandValue::Ref(..) => {
                                bug!("by-ref operand {:?} in trans_rvalue_operand", operand);
                            }
                        }
                    }
                    ...
                    mir::CastKind::Misc => {
                        ...
                        let newval = match (r_t_in, r_t_out) {
                            ...
                            | (CastTy::RPtr(_), CastTy::Ptr(_)) => bcx.pointercast(llval, ll_t_out),
                            (CastTy::Ptr(_), CastTy::Int(_)) | (CastTy::FnPtr, CastTy::Int(_)) => {
                                bcx.ptrtoint(llval, ll_t_out)
                            }
                            (CastTy::Int(_), CastTy::Ptr(_)) => bcx.inttoptr(llval, ll_t_out),
                ...
                (bcx, OperandRef { val, layout: cast })
            }
            ...
            mir::Rvalue::BinaryOp(op, ref lhs, ref rhs) => {
                ...
                let operand = OperandRef {
                    ...
                    layout: bcx.ccx
                        .layout_of(op.ty(bcx.tcx(), lhs.layout.ty, rhs.layout.ty)),
                };
                (bcx, operand)
            }
            mir::Rvalue::CheckedBinaryOp(op, ref lhs, ref rhs) => {
                let lhs = self.trans_operand(&bcx, lhs);
                let rhs = self.trans_operand(&bcx, rhs);
                let result = self.trans_scalar_checked_binop(
                    &bcx,
                    op,
                    lhs.immediate(),
                    rhs.immediate(),
            ...
            mir::Rvalue::Repeat(..) | mir::Rvalue::Aggregate(..) => {
                ...
                (
                    bcx,
                    OperandRef::new_zst(self.ccx, self.ccx.layout_of(self.monomorphize(&ty))),
                )
            }
        }
    }
    ...
    pub fn trans_scalar_binop(
        ...
        match op {
            ...
            | mir::BinOp::Ge => if is_nil {
                ...
                bcx.fcmp(base::bin_op_to_fcmp_predicate(op.to_hir_binop()), lhs, rhs)
            } else {
    ...
    pub fn trans_scalar_checked_binop(
        &mut self,
        bcx: &Builder<'a, 'tcx>,
        op: mir::BinOp,
        lhs: ValueRef,
        ...
        let (val, of) = match op {
            ...
            mir::BinOp::Add | mir::BinOp::Sub | mir::BinOp::Mul => {
                ...
                (bcx.extract_value(res, 0), bcx.extract_value(res, 1))
            }
