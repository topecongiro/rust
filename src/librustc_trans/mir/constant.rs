...
use callee;
use builder::Builder;
use common::{self, const_get_elt, val_ty, CrateContext};
...
impl<'a, 'tcx> Const<'tcx> {
    ...
    pub fn from_constval(ccx: &CrateContext<'a, 'tcx>, cv: &ConstVal, ty: Ty<'tcx>) -> Const<'tcx> {
    ...
    pub fn to_operand(&self, ccx: &CrateContext<'a, 'tcx>) -> OperandRef<'tcx> {
        ...
        let val = if llty == llvalty && layout.is_llvm_scalar_pair() {
            ...
            OperandValue::Ref(
                consts::ptrcast(ptr, layout.llvm_type(ccx).ptr_to()),
...
impl<'tcx> ConstLvalue<'tcx> {
    ...
    pub fn len<'a>(&self, ccx: &CrateContext<'a, 'tcx>) -> ValueRef {
        match self.ty.sty {
            ty::TyArray(_, n) => C_usize(ccx, n.val.to_const_int().unwrap().to_u64().unwrap()),
    ...
    fn const_lvalue(
        ...
        if let mir::Lvalue::Local(index) = *lvalue {
            return self.locals[index]
                .clone()
                .unwrap_or_else(|| span_bug!(span, "{:?} not initialized", lvalue))
                .map(|v| v.as_lvalue());
        }

        let lvalue = match *lvalue {
            mir::Lvalue::Local(_) => bug!(), // handled above
            mir::Lvalue::Static(box mir::Static { def_id, ty }) => ConstLvalue {
            ...
            mir::Lvalue::Projection(ref projection) => {
                ...
                let projected_ty =
                    LvalueTy::Ty { ty: tr_base.ty }.projection_ty(tcx, &projection.elem);
            ...
            mir::Rvalue::Cast(ref kind, ref source, cast_ty) => {
                ...
                let val = match *kind {
                    ...
                    mir::CastKind::ClosureFnPointer => {
                        match operand.ty.sty {
                            ty::TyClosure(def_id, substs) => {
                                ...
                                let input = tcx.fn_sig(def_id).subst(tcx, substs.substs).input(0);
                    ...
                    mir::CastKind::Unsize => {
                        ...
                        let info = base::unsized_info(self.ccx, pointee_ty, unsized_ty, old_info);

                        if old_info.is_none() {
                            let prev_const = self.ccx
                                .const_unsized()
                                .borrow_mut()
                    ...
                    mir::CastKind::Misc if self.ccx.layout_of(operand.ty).is_llvm_immediate() => {
                        ...
                        unsafe {
                            match (r_t_in, r_t_out) {
                                ...
                                (CastTy::Float, CastTy::Int(IntTy::I)) => cast_const_float_to_int(
                                    self.ccx,
                                    &operand,
                                    true,
                                    ll_t_out,
...
pub fn trans_static_initializer<'a, 'tcx>(
    ...
    MirConstContext::trans_def(ccx, def_id, Substs::empty(), IndexVec::new()).map(|c| c.llval)
}
        ...
        layout::Variants::Tagged { .. } => {
            let discr = match *kind {
                mir::AggregateKind::Adt(adt_def, _, _, _) => adt_def
                    .discriminant_for_variant(ccx.tcx(), variant_index)
                    .to_u128_unchecked()
                    as u64,
                _ => 0,
            };
...
fn build_const_struct<'a, 'tcx>(
    ...
    match layout.abi {
        layout::Abi::Scalar(_) | layout::Abi::ScalarPair(..) if discr.is_none() => {
