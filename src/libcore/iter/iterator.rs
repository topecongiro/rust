...
use super::{Chain, Cloned, Cycle, Enumerate, Filter, FilterMap, Fuse};
...
#[rustc_on_unimplemented(
    on(_Self = "&str", label = "`{Self}` is not an iterator; try calling `.chars()` or `.bytes()`"),
...
pub trait Iterator {
    ...
    /// ```
    #[inline]
    #[unstable(feature = "iterator_find_map", reason = "unstable new API", issue = "49602")]
