...
/// [`Iterator`]: trait.Iterator.html
#[must_use = "iterator adaptors are lazy and do nothing unless consumed"]
#[unstable(feature = "iterator_step_by", reason = "unstable replacement of Range::step_by",
           issue = "27741")]
#[derive(Clone, Debug)]
pub struct StepBy<I> {
    iter: I,
    ...
    B: Iterator,
{
    ...
    default fn next(&mut self) -> Option<(A::Item, B::Item)> {
        self.a
            .next()
            .and_then(|x| self.b.next().and_then(|y| Some((x, y))))
    }
    ...
    F: FnMut(&mut St, I::Item) -> Option<B>,
{
        ...
        R: Try<Ok = Acc>,
    {
        ...
        let f = &mut self.f;
        self.iter
            .try_fold(init, move |acc, x| match f(state, x) {
    ...
    F: FnMut(I::Item) -> U,
{
    ...
    #[inline]
    fn size_hint(&self) -> (usize, Option<usize>) {
        let (flo, fhi) = self.frontiter
            .as_ref()
            .map_or((0, Some(0)), |it| it.size_hint());
        let (blo, bhi) = self.backiter
            .as_ref()
            .map_or((0, Some(0)), |it| it.size_hint());
