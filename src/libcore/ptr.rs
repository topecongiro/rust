...
unsafe fn swap_nonoverlapping_bytes(x: *mut u8, y: *mut u8, len: usize) {
    ...
    #[cfg_attr(
        not(any(target_os = "emscripten", target_os = "redox", target_endian = "big")), repr(simd)
    )]
...
impl<T: ?Sized> *const T {
        ...
        T: Sized,
    {
        ...
        unsafe { intrinsics::align_offset(self as *const (), align) }
    }
}
...
impl<T: ?Sized> NonNull<T> {
    ...
    pub fn as_opaque(self) -> NonNull<::alloc::Opaque> {
        unsafe { NonNull::new_unchecked(self.as_ptr() as _) }
    }
}
