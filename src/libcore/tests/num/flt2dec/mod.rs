...
use std::prelude::v1::*;
use std::{fmt, mem, str, f32, f64, i16};
...
use core::num::flt2dec::{to_exact_exp_str, to_exact_fixed_str, to_shortest_exp_str,
                         to_shortest_str};
    ...
    F: FnMut(&Decoded, &mut [u8], i16) -> (usize, i16),
{
    ...
    for i in 1..cut.unwrap_or(expected.len() - 1) {
        ...
        if expected[i] >= b'5' {
            ...
            if !(i + 1 < expected.len() && expected[i - 1] & 1 == 0 && expected[i] == b'5'
                && expected[i + 1] == b' ')
            {
    ...
    G: FnMut(&Decoded, &mut [u8]) -> (usize, i16),
{
    ...
    let (npassed, nignored) = iterate(
        "f32_exhaustive_equivalence_test",
        k,
        0x7f7f_ffff,
        f,
        g,
    ...
    F: FnMut(&Decoded, &mut [u8]) -> (usize, i16),
{
    ...
    assert_eq!(
        to_string(f, 1.9971e20, Minus, 0, false),
        "199710000000000000000"
    );
    assert_eq!(
        to_string(f, 1.9971e20, Minus, 1, false),
        "199710000000000000000.0"
    );
    assert_eq!(
        to_string(f, 1.9971e20, Minus, 8, false),
        "199710000000000000000.00000000"
    );

    assert_eq!(
        to_string(f, f32::MAX, Minus, 0, false),
    ...
    assert_eq!(
        to_string(f, f32::MAX, Minus, 8, false),
        format!("34028235{:0>31}.00000000", "")
    );

    let minf32 = ldexp_f32(1.0, -149);
    assert_eq!(
        to_string(f, minf32, Minus, 0, false),
        format!("0.{:0>44}1", "")
    );
    assert_eq!(
        to_string(f, minf32, Minus, 45, false),
        format!("0.{:0>44}1", "")
    );
    assert_eq!(
        to_string(f, minf32, Minus, 46, false),
        format!("0.{:0>44}10", "")
    );
    ...
    assert_eq!(
        to_string(f, f64::MAX, Minus, 1, false),
    ...
    let minf64 = ldexp_f64(1.0, -1074);
    assert_eq!(
        to_string(f, minf64, Minus, 0, false),
        format!("0.{:0>323}5", "")
    );
    assert_eq!(
        to_string(f, minf64, Minus, 324, false),
        format!("0.{:0>323}5", "")
    );
    assert_eq!(
        to_string(f, minf64, Minus, 325, false),
        format!("0.{:0>323}50", "")
    );

    // very large output
    assert_eq!(
        to_string(f, 1.1, Minus, 80000, false),
        format!("1.1{:0>79999}", "")
    );
}
    ...
    F: FnMut(&Decoded, &mut [u8]) -> (usize, i16),
{
    ...
    assert_eq!(
        to_string(f, 1.0 / 0.0, MinusPlusRaw, (-4, 16), true),
        "+inf"
    );
    ...
    F: FnMut(&Decoded, &mut [u8], i16) -> (usize, i16),
{
    ...
    // very large output
    assert_eq!(
        to_string(f, 0.0, Minus, 80000, false),
        format!("0.{:0>79999}e0", "")
    );
    assert_eq!(
        to_string(f, 1.0e1, Minus, 80000, false),
        format!("1.{:0>79999}e1", "")
    );
    assert_eq!(
        to_string(f, 1.0e0, Minus, 80000, false),
        format!("1.{:0>79999}e0", "")
    );
    assert_eq!(
    ...
    F: FnMut(&Decoded, &mut [u8], i16) -> (usize, i16),
{
    ...
    assert_eq!(
        to_string(f, 999.5, Minus, 30, false),
    ...
    assert_eq!(
        to_string(f, 0.95, Minus, 30, false),
    ...
    assert_eq!(
        to_string(f, 0.095, Minus, 30, false),
    ...
    assert_eq!(
        to_string(f, 0.0095, Minus, 30, false),
    ...
    assert_eq!(to_string(f, 7.5e-11, Minus, 13, false), "0.0000000000750");
    assert_eq!(
        to_string(f, 7.5e-11, Minus, 20, false),
        "0.00000000007500000000"
    );
    assert_eq!(
        to_string(f, 7.5e-11, Minus, 30, false),
        "0.000000000074999999999999999501"
    );

    assert_eq!(
        to_string(f, 1.0e25, Minus, 0, false),
        "10000000000000000905969664"
    );
    assert_eq!(
        to_string(f, 1.0e25, Minus, 1, false),
        "10000000000000000905969664.0"
    );
    assert_eq!(
        to_string(f, 1.0e25, Minus, 3, false),
    ...
    assert_eq!(
        to_string(f, 1.0e-6, Minus, 22, false),
        "0.0000010000000000000000"
    );
    assert_eq!(
        to_string(f, 1.0e-6, Minus, 23, false),
        "0.00000099999999999999995"
    );
    assert_eq!(
        to_string(f, 1.0e-6, Minus, 24, false),
        "0.000000999999999999999955"
    );
    assert_eq!(
        to_string(f, 1.0e-6, Minus, 25, false),
        "0.0000009999999999999999547"
    );
    assert_eq!(
        to_string(f, 1.0e-6, Minus, 35, false),
    ...
    assert_eq!(
        to_string(f, 1.0e-1, Minus, 80000, false),
        format!(
            "0.1000000000000000055511151231257827021181583404541015625{:0>79945}",
            ""
        )
    );
    assert_eq!(
