...
use rustc::lint;
use rustc::lint::builtin::{ABSOLUTE_PATHS_NOT_STARTING_WITH_CRATE, BARE_TRAIT_OBJECTS};
use rustc::session;
use rustc::util;
...
pub fn register_builtins(store: &mut lint::LintStore, sess: Option<&Session>) {
    ...
    add_builtin_with_new!(sess, TypeLimits, MissingDoc, MissingDebugImplementations,);
    ...
    store.register_future_incompatible(
        sess,
        vec![
    ...
    store.register_removed(
        "fat_ptr_transmutes",
        "was accidentally removed back in 2014",
    );
