...
impl<'a, 'tcx> LateLintPass<'a, 'tcx> for TypeLimits {
    fn check_expr(&mut self, cx: &LateContext<'a, 'tcx>, e: &'tcx hir::Expr) {
                    ...
                    if let Some(bits) = opt_ty_bits {
                        let exceeding = if let hir::ExprLit(ref lit) = r.node {
                            ...
                            match const_cx.eval(&r) {
                                Ok(&ty::Const {
                                    val: ConstVal::Integral(i),
                                    ..
                                }) => {
                                    i.is_negative() || i.to_u64().map(|i| i >= bits).unwrap_or(true)
                                }
                                _ => false,
                            }
                        };
        ...
        fn check_limits(cx: &LateContext, binop: hir::BinOp, l: &hir::Expr, r: &hir::Expr) -> bool {
...
fn is_ffi_safe(ty: attr::IntType) -> bool {
    match ty {
        attr::SignedInt(ast::IntTy::I8)
        | attr::UnsignedInt(ast::UintTy::U8)
        | attr::SignedInt(ast::IntTy::I16)
        | attr::UnsignedInt(ast::UintTy::U16)
        | attr::SignedInt(ast::IntTy::I32)
        | attr::UnsignedInt(ast::UintTy::U32)
        | attr::SignedInt(ast::IntTy::I64)
        | attr::UnsignedInt(ast::UintTy::U64)
        | attr::SignedInt(ast::IntTy::I128)
...
impl<'a, 'tcx> ImproperCTypesVisitor<'a, 'tcx> {
    ...
    fn check_type_for_ffi(&self, cache: &mut FxHashSet<Ty<'tcx>>, ty: Ty<'tcx>) -> FfiResult {
        ...
        match ty.sty {
                ...
                match def.adt_kind() {
                    AdtKind::Struct => {
                        ...
                        for field in &def.non_enum_variant().fields {
                            let field_ty =
                                cx.fully_normalize_associated_types_in(&field.ty(cx, substs));
                            // repr(transparent) types are allowed to have arbitrary ZSTs, not just
                            // PhantomData -- skip checking all ZST fields
                            if def.repr.transparent() {
                                let is_zst = cx.layout_of(cx.param_env(field.did).and(field_ty))
            ...
            ty::TyChar => FfiUnsafe(
                "found Rust type `char` in foreign module, while \
                 `u32` or `libc::wchar_t` should be used",
            ),

            ty::TyInt(ast::IntTy::I128) => FfiUnsafe(
                "found Rust type `i128` in foreign module, but \
                 128-bit integers don't currently have a known \
                 stable ABI",
            ),

            ty::TyUint(ast::UintTy::U128) => FfiUnsafe(
                "found Rust type `u128` in foreign module, but \
                 128-bit integers don't currently have a known \
                 stable ABI",
            ),
            ...
            ty::TySlice(_) => FfiUnsafe(
                "found Rust slice type in foreign module, \
                 consider using a raw pointer instead",
            ),

            ty::TyDynamic(..) => FfiUnsafe(
                "found Rust trait type in foreign module, \
                 consider using a raw pointer instead",
            ),

            ty::TyStr => FfiUnsafe(
                "found Rust type `str` in foreign module; \
            ...
            ty::TyFnPtr(sig) => {
                match sig.abi() {
                    Abi::Rust | Abi::RustIntrinsic | Abi::PlatformIntrinsic | Abi::RustCall => {
                        return FfiUnsafe(
                            "found function pointer with Rust calling convention in \
        ...
        match self.check_type_for_ffi(&mut FxHashSet(), ty) {
            ...
            FfiResult::FfiPhantom => {
                self.cx.span_lint(
                    ...
                    sp,
                    &format!(
                        "found zero-sized type composed only \
            ...
            FfiResult::FfiBadEnum(_, s) => {
                ...
                self.cx.span_lint(
                    IMPROPER_CTYPES,
                    sp,
                    &format!("found non-foreign-function-safe member in enum: {}", s),
                );
            }
        }
    }
