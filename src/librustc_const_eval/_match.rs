...
impl<'tcx> PatternFolder<'tcx> for LiteralExpander {
    fn fold_pattern(&mut self, pat: &Pattern<'tcx>) -> Pattern<'tcx> {
        match (&pat.ty.sty, &*pat.kind) {
            (&ty::TyRef(_, mt), &PatternKind::Constant { ref value }) => Pattern {
                ty: pat.ty,
                span: pat.span,
...
impl<'a, 'tcx> fmt::Debug for Matrix<'a, 'tcx> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let pretty_printed_matrix: Vec<Vec<String>> = m.iter()
...
impl<'a, 'tcx> MatchCheckCtxt<'a, 'tcx> {
    pub fn create_and_enter<F, R>(tcx: TyCtxt<'a, 'tcx, 'tcx>, module: DefId, f: F) -> R
    where
        ...
        'a: 'p,
    {
        ...
        self.byte_array_map
            .entry(pat)
            .or_insert_with(|| match pat.kind {
                box PatternKind::Constant {
                    value:
                        &ty::Const {
                            val: ConstVal::ByteStr(b),
                            ..
                        },
                } => b.data
                    .iter()
    ...
    fn apply_constructor<'a>(
        ...
        let pat = {
            ...
            match ty.sty {
                ...
                ty::TyRef(..) => PatternKind::Deref {
                    subpattern: pats.nth(0).unwrap(),
                },

                ty::TySlice(_) | ty::TyArray(..) => PatternKind::Slice {
                    prefix: pats.collect(),
                    slice: None,
                    suffix: vec![],
                },
...
fn all_constructors<'a, 'tcx: 'a>(
    ...
    match pcx.ty.sty {
        ...
        ty::TyArray(ref sub_ty, _) | ty::TySlice(ref sub_ty) => {
        ...
        ty::TyAdt(def, substs) if def.is_enum() => def.variants
            .iter()
...
fn max_slice_length<'p, 'a: 'p, 'tcx: 'a, I>(_cx: &mut MatchCheckCtxt<'a, 'tcx>, patterns: I) -> u64
where
...
pub fn is_useful<'p, 'a: 'p, 'tcx: 'a>(
    ...
    if let Some(constructors) = pat_constructors(cx, v[0], pcx) {
        ...
        constructors
            .into_iter()
            .map(|c| is_useful_specialized(cx, matrix, v, c.clone(), pcx.ty, witness))
            .find(|result| result.is_useful())
            .unwrap_or(NotUseful)
    } else {
        debug!("is_useful - expanding wildcard");

        let used_ctors: Vec<Constructor> = rows.iter()
            .flat_map(|row| pat_constructors(cx, row[0], pcx).unwrap_or(vec![]))
            .collect();
        debug!("used_ctors = {:?}", used_ctors);
        let all_ctors = all_constructors(cx, pcx);
        debug!("all_ctors = {:?}", all_ctors);
        let missing_ctors: Vec<Constructor> = all_ctors
            .iter()
            .filter(|c| !used_ctors.contains(*c))
            .cloned()
            .collect();
...
fn constructor_arity(_cx: &MatchCheckCtxt, ctor: &Constructor, ty: Ty) -> u64 {
    ...
    match ty.sty {
        ...
        ty::TyRef(..) => 1,
        ty::TyAdt(adt, _) => adt.variants[ctor.variant_index_for_adt(adt)].fields.len() as u64,
        _ => 0,
    }
}
...
fn specialize<'p, 'a: 'p, 'tcx: 'a>(
    ...
    let head: Option<Vec<&Pattern>> = match *pat.kind {
        PatternKind::Binding { .. } | PatternKind::Wild => Some(wild_patterns.to_owned()),
        ...
        PatternKind::Constant { value } => match *constructor {
            ...
            _ => match constructor_covered_by_range(
                cx.tcx,
                pat.span,
                constructor,
                &value.val,
        ...
        PatternKind::Range { lo, hi, ref end } => match constructor_covered_by_range(
            cx.tcx,
            pat.span,
            constructor,
            &lo.val,
        ...
        | PatternKind::Slice {
            ...
            ConstantValue(..) => match slice_pat_covered_by_constructor(
                cx.tcx,
                pat.span,
                constructor,
                prefix,
