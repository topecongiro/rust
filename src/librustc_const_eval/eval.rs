...
use rustc::middle::const_val::{ByteArray, ConstEvalErr, ConstVal, ErrKind, EvalResult};
...
pub fn lookup_const_by_id<'a, 'tcx>(
    tcx: TyCtxt<'a, 'tcx, 'tcx>,
    key: ty::ParamEnvAnd<'tcx, (DefId, &'tcx Substs<'tcx>)>,
) -> Option<(DefId, &'tcx Substs<'tcx>)> {
    ty::Instance::resolve(tcx, key.param_env, key.value.0, key.value.1)
...
fn eval_const_expr_partial<'a, 'tcx>(
    cx: &ConstContext<'a, 'tcx>,
    e: &'tcx Expr,
) -> EvalResult<'tcx> {
    ...
    let result = match e.node {
        hir::ExprUnary(hir::UnNeg, ref inner) => {
            ...
            if let hir::ExprLit(ref lit) = inner.node {
                ...
                let negated = match (&lit.node, &ty.sty) {
                    ...
                    | (&LitKind::Int(n, Signed(IntTy::Is)), _) => match tcx.sess.target.isize_ty {
        ...
        hir::ExprUnary(hir::UnNot, ref inner) => mk_const(match cx.eval(inner)?.val {
            Integral(i) => Integral(math!(e, !i)),
            Bool(b) => Bool(!b),
            _ => signal!(e, TypeckError),
        }),
        ...
        hir::ExprBinary(op, ref a, ref b) => {
            ...
            mk_const(match (cx.eval(a)?.val, cx.eval(b)?.val) {
                ...
                (Bool(a), Bool(b)) => Bool(match op.node {
                    hir::BiAnd => a && b,
                    hir::BiOr => a || b,
                    hir::BiBitXor => a ^ b,
                    hir::BiBitAnd => a & b,
                ...
                (Char(a), Char(b)) => Bool(match op.node {
                    hir::BiEq => a == b,
                    hir::BiNe => a != b,
                    hir::BiLt => a < b,
                    hir::BiLe => a <= b,
        ...
        hir::ExprPath(ref qpath) => {
            ...
            match cx.tables.qpath_def(qpath, e.hir_id) {
                ...
                Def::VariantCtor(variant_def, CtorKind::Const) => mk_const(Variant(variant_def)),
                Def::VariantCtor(_, CtorKind::Fn) => {
                    signal!(e, UnimplementedConstVal("enum variants"));
                }
                Def::StructCtor(_, CtorKind::Const) => mk_const(Aggregate(Struct(&[]))),
            ...
            if tcx.fn_sig(def_id).abi() == Abi::RustIntrinsic {
                ...
                match &tcx.item_name(def_id)[..] {
                    "size_of" => {
                        ...
                        return Ok(mk_const(Integral(Usize(
                            ConstUsize::new(size, tcx.sess.target.usize_ty).unwrap(),
                        ))));
                    }
                    "min_align_of" => {
                        let align = layout_of(substs.type_at(0))?.align.abi();
                        return Ok(mk_const(Integral(Usize(
                            ConstUsize::new(align, tcx.sess.target.usize_ty).unwrap(),
                        ))));
                    }
                    _ => signal!(e, TypeckError),
                }
        ...
        hir::ExprStruct(_, ref fields, _) => mk_const(Aggregate(Struct(
            tcx.alloc_name_const_slice(&fields
                .iter()
                .map(|f| cx.eval(&f.expr).map(|v| (f.name.node, v)))
                .collect::<Result<Vec<_>, _>>()?),
        ))),
        hir::ExprIndex(ref arr, ref idx) => {
            ...
            match arr.val {
                ...
                ByteStr(b) if idx >= b.data.len() as u64 => signal!(
                    e,
                    IndexOutOfBounds {
                ...
                ByteStr(b) => mk_const(Integral(U8(b.data[idx as usize]))),
...
fn cast_const_int<'a, 'tcx>(
    ...
    match ty.sty {
        ...
        ty::TyInt(ast::IntTy::Is) => Ok(Integral(Isize(ConstIsize::new_truncating(
            v as i128,
        ...
        ty::TyUint(ast::UintTy::Us) => Ok(Integral(Usize(ConstUsize::new_truncating(
            v,
        ...
        ty::TyRawPtr(_) => Err(ErrKind::UnimplementedConstVal(
            "casting an address to a raw ptr",
        )),
        ty::TyChar => match val {
...
fn cast_const_float<'a, 'tcx>(
    tcx: TyCtxt<'a, 'tcx, 'tcx>,
    val: ConstFloat,
    ty: Ty<'tcx>,
) -> CastResult<'tcx> {
    let int_width = |ty| ty::layout::Integer::from_attr(tcx, ty).size().bits() as usize;
    match ty.sty {
        ty::TyInt(ity) => {
    ...
    match val {
        ...
        ByteStr(b) => match ty.sty {
            ...
            ty::TyRef(
                ...
                ty::TySlice(_) => Err(ErrKind::UnimplementedConstVal("casting a bytestr to slice")),
                _ => Err(CannotCast),
            },
            _ => Err(CannotCast),
        },
...
fn lit_to_const<'a, 'tcx>(
    ...
    match *lit {
        ...
        LitKind::Int(n, hint) => match (&ty.sty, hint) {
            (&ty::TyInt(ity), _) | (_, Signed(ity)) => Ok(Integral(
                ConstInt::new_signed_truncating(n as i128, ity, tcx.sess.target.isize_ty),
            )),
            (&ty::TyUint(uty), _) | (_, Unsigned(uty)) => Ok(Integral(
                ConstInt::new_unsigned_truncating(n as u128, uty, tcx.sess.target.usize_ty),
            )),
            _ => bug!(),
        },
        LitKind::Float(n, fty) => parse_float(&n.as_str(), fty).map(Float),
...
fn parse_float<'tcx>(num: &str, fty: ast::FloatTy) -> Result<ConstFloat, ErrKind<'tcx>> {
