...
use super::{Error, ObligationForest, ObligationProcessor, Outcome};
...
where
    OF: FnMut(&mut O) -> Result<Option<Vec<O>>, &'static str>,
    BF: FnMut(&[O]),
{
...
fn push_pop() {
    ...
    let Outcome {
        ...
        errors: err,
        ..
    } = forest.process_obligations(&mut C(
...
fn done_dependency() {
    ...
    let Outcome {
        ...
        |obligation| match *obligation {
            "(A,B,C): Sized" => Ok(Some(vec!["A: Sized", "B: Sized", "C: Sized"])),
            _ => unreachable!(),
        },
        |_| {},
    ));
