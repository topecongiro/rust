...
impl Step for Std {
    ...
    fn run(self, builder: &Builder) {
        ...
        run_cargo(build, &mut cargo, &libstd_stamp(build, compiler, target));
...
impl Step for StartupObjects {
    ...
    fn run(self, builder: &Builder) {
        ...
        for obj in ["crt2.o", "dllcrt2.o"].iter() {
            copy(
                &compiler_file(build.cc(target), obj),
                &sysroot_dir.join(obj),
            );
        }
    }
}
...
impl Step for Test {
    ...
    fn run(self, builder: &Builder) {
        ...
        run_cargo(build, &mut cargo, &libtest_stamp(build, compiler, target));
...
impl Step for Rustc {
    ...
    fn run(self, builder: &Builder) {
        ...
        run_cargo(build, &mut cargo, &librustc_stamp(build, compiler, target));
...
pub fn rustc_cargo(
    ...
    cargo
        ...
        .env(
            "CFG_PREFIX",
            build.config.prefix.clone().unwrap_or_default(),
        );

    if compiler.stage == 0 {
        cargo.env("CFG_LIBDIR_RELATIVE", "lib");
    } else {
        let libdir_relative = build
            .config
            .libdir_relative
            .clone()
            .unwrap_or(PathBuf::from("lib"));
    ...
    if build.config.llvm_static_stdcpp && !target.contains("windows") && !target.contains("apple") {
        cargo.env(
            "LLVM_STATIC_STDCPP",
