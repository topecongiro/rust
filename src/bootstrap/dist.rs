...
use channel;
use util::{copy, cp_filtered, cp_r, is_dylib, libdir};
...
impl Step for Rustc {
    ...
    fn run(self, builder: &Builder) -> PathBuf {
        ...
        fn prepare_image(builder: &Builder, compiler: Compiler, image: &Path) {
            ...
            cp_r(
                &build.src.join("src/doc/man"),
                &image.join("share/man/man1"),
            );
...
impl Step for DebuggerScripts {
    ...
    fn run(self, builder: &Builder) {
        ...
        if host.contains("windows-msvc") {
            ...
            install(
                &build.src.join("src/etc/rust-windbg.cmd"),
                &sysroot.join("bin"),
                0o755,
            );
            ...
            install(
                &build.src.join("src/etc/rust-gdb"),
                &sysroot.join("bin"),
                0o755,
            );
            ...
            install(
                &build.src.join("src/etc/rust-lldb"),
                &sysroot.join("bin"),
                0o755,
            );
...
fn copy_src_dirs(build: &Build, src_dirs: &[&str], exclude_dirs: &[&str], dst_dir: &Path) {
    ...
    for item in src_dirs {
        ...
        t!(fs::create_dir_all(dst));
        cp_filtered(&build.src.join(item), dst, &|path| {
