...
impl Step for Std {
    ...
    fn run(self, builder: &Builder) -> PathBuf {
        ...
        builder.cp_filtered(&src, &dst, &|path| {
            ...
            name != Some(builder.config.rust_codegen_backends_dir.as_str()) && name != Some("bin")
        });
...
fn copy_src_dirs(builder: &Builder, src_dirs: &[&str], exclude_dirs: &[&str], dst_dir: &Path) {
    ...
    for item in src_dirs {
        ...
        t!(fs::create_dir_all(dst));
        builder.cp_filtered(&builder.src.join(item), dst, &|path| {
    ...
    fn run(self, builder: &Builder) {
        ...
        if target.contains("windows") {
            ...
            let prepare = |name: &str| {
                ...
                builder.cp_r(
                    &work
                        .join(&format!("{}-{}", pkgname(builder, name), target))
                        .join(dir),
                    &exe.join(name),
                );
