    ...
    fn visit_item(&mut self, it: &'tcx hir::Item) {
        ...
        for m in it.attrs.iter().filter(|a| a.check_name("link")) {
            ...
            let kind = match kind.as_ref().map(|s| &s[..]) {
                ...
                Some(k) => {
                    struct_span_err!(self.tcx.sess, m.span, E0458, "unknown kind: `{}`", k)
                        .span_label(m.span, "unknown kind")
                        .emit();
            ...
            let cfg = items
                .iter()
                .find(|k| k.check_name("cfg"))
                .and_then(|a| a.meta_item_list());
            let cfg = if let Some(list) = cfg {
                if list.is_empty() {
                    self.tcx
                        .sess
                        .span_err(m.span(), "`cfg()` must have an argument");
                    return;
                    ...
                    self.tcx
                        .sess
                        .span_err(list[0].span(), "invalid argument for `cfg(..)`");
                    return;
                }
            } else {
                None
            };
            let foreign_items = fm.items
                .iter()
        ...
        for &(ref name, ref new_name, kind) in &self.tcx.sess.opts.libs {
            ...
            for lib in self.libs.iter_mut() {
                if lib.name == name as &str {
                    ...
                    if !changed {
                        let msg = format!(
                            "redundant linker flag specified for \
                             library `{}`",
                            name
                        );
