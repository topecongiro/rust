...
use creader::Library;
use schema::{rustc_version, METADATA_HEADER};
...
impl CratePaths {
    fn paths(&self) -> Vec<PathBuf> {
        self.dylib
            .iter()
            .chain(self.rlib.iter())
            .chain(self.rmeta.iter())
            .cloned()
            .collect()
    }
}
...
impl<'a> Context<'a> {
    ...
    pub fn report_errs(&mut self) -> ! {
        ...
        let mut err = if !self.rejected_via_hash.is_empty() {
            ...
            match self.root {
                &None => {}
                &Some(ref r) => for path in r.paths().iter() {
            ...
            if (self.ident == "std" || self.ident == "core") && self.triple != config::host_triple()
            {
                err.note(&format!(
    ...
    fn find_library_crate(&mut self) -> Option<Library> {
        ...
        self.filesearch.search(|path, kind| {
            ...
            let (hash, found_kind) = if file.starts_with(&rlib_prefix) && file.ends_with(".rlib") {
                ...
                (
                    &file[(rlib_prefix.len())..(file.len() - ".rmeta".len())],
                    CrateFlavor::Rmeta,
                )
    ...
    fn extract_one(
        ...
        for (lib, kind) in m {
            ...
            if error > 0 {
                error += 1;
                err.as_mut().unwrap().span_note(
                    self.span,
                    &format!(r"candidate #{}: {}", error, lib.display()),
                );
                continue;
            }
            ...
            if let Some((ref prev, _)) = ret {
                ...
                let sysroot = sysroot.canonicalize().unwrap_or(sysroot.to_path_buf());
    ...
    fn staticlibname(&self) -> (String, String) {
        let t = &self.target;
        (
            t.options.staticlib_prefix.clone(),
            t.options.staticlib_suffix.clone(),
        )
    }

    fn find_commandline_library<'b, LOCS>(&mut self, locs: LOCS) -> Option<Library>
    where
        LOCS: Iterator<Item = &'b String>,
    {
    ...
    let raw_bytes: ErasedBoxRef<[u8]> = match flavor {
        ...
        CrateFlavor::Rmeta => {
            let mut file = File::open(filename)
                .map_err(|_| format!("could not open file: '{}'", filename.display()))?;
            let mut buf = vec![];
            file.read_to_end(&mut buf)
                .map_err(|_| format!("failed to read rmeta metadata: '{}'", filename.display()))?;
