    ...
    fn uniform(
        ...
        match proj.elem {
            ...
            ProjectionElem::Subslice { from, to } => {
                ...
                let temps: Vec<_> = (from..(size - to))
                    .map(|i| {
                        let temp = self
                            .patch
                            .new_temp(item_ty, self.mir.source_info(location).span);
                        self.patch
                            .add_statement(location, StatementKind::StorageLive(temp));
                ...
                self.patch.add_assign(
                    ...
                    Rvalue::Aggregate(
                        ...
                        temps
                            .iter()
                            .map(|x| Operand::Move(Place::Local(*x)))
                            .collect(),
                    ),
                );
                for temp in temps {
                    self.patch
                        .add_statement(location, StatementKind::StorageDead(temp));
                }
            }
                            ...
                            .map(|item| {
                                if let Operand::Move(Place::Local(local)) = item {
                                    ...
                                    let opt_index_and_place =
                                        Self::try_get_item_source(local_use, mir);
...
impl RestoreSubsliceArrayMoveOut {
    ...
    fn check_and_patch<'tcx>(
        ...
        if opt_size.is_some()
            && items
                .iter()
                .all(|l| l.is_some() && l.unwrap().2 == opt_src_place.unwrap())
        {
