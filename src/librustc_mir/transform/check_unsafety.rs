...
impl<'a, 'tcx> Visitor<'tcx> for UnsafetyChecker<'a, 'tcx> {
    ...
    fn visit_statement(
        ...
        match statement.kind {
            ...
            StatementKind::InlineAsm { .. } => self.require_unsafe("use of inline assembly"),
        }
    ...
    fn visit_place(
        ...
        if let PlaceContext::Borrow { .. } = context {
            if util::is_disaligned(self.tcx, self.mir, self.param_env, place) {
                ...
                let lint_root = self.visibility_scope_info[source_info.scope].lint_root;
        ...
        match place {
            &Place::Projection(box Projection { ref base, ref elem }) => {
                ...
                match base_ty.sty {
                    ty::TyRawPtr(..) => self.require_unsafe("dereference of raw pointer"),
                    ty::TyAdt(adt, _) => {
                        if adt.is_union() {
                            if context == PlaceContext::Store || context == PlaceContext::AsmOutput
                                || context == PlaceContext::Drop
                            {
                                ...
                                if elem_ty.moves_by_default(
                                    ...
                                    self.require_unsafe("assignment to non-`Copy` union field")
                                } else {
...
impl<'a, 'tcx> UnsafetyChecker<'a, 'tcx> {
    fn require_unsafe(&mut self, description: &'static str) {
        let source_info = self.source_info;
        self.register_violations(
            &[
                UnsafetyViolation {
    ...
    fn register_violations(
        ...
        self.inherited_blocks.extend(
            unsafe_blocks
                .iter()
                .map(|&(node_id, is_used)| (node_id, is_used && !within_unsafe)),
        );
    }
}
...
impl<'a, 'tcx> hir::intravisit::Visitor<'tcx> for UnusedUnsafeVisitor<'a> {
    ...
    fn visit_block(&mut self, block: &'tcx hir::Block) {
        ...
        if let hir::UnsafeBlock(hir::UserProvided) = block.rules {
            self.unsafe_blocks
                .push((block.id, self.used_unsafe.contains(&block.id)));
        }
    }
}
...
fn check_unused_unsafe<'a, 'tcx>(
    ...
    let body_id = tcx.hir
        ...
        .and_then(|node_id| tcx.hir.maybe_body_owned_by(node_id));
    ...
    debug!(
        "check_unused_unsafe({:?}, body={:?}, used_unsafe={:?})",
...
fn unsafety_check_result<'a, 'tcx>(
    tcx: TyCtxt<'a, 'tcx, 'tcx>,
    def_id: DefId,
) -> UnsafetyCheckResult {
    ...
    let mut checker = UnsafetyChecker::new(mir, visibility_scope_info, tcx, param_env);
...
fn unsafe_derive_on_repr_packed<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) {
    ...
    let message = if !tcx.generics_of(def_id).types.is_empty() {
        ...
        format!(
            "#[derive] can't be used on a non-Copy #[repr(packed)] struct \
             (error E0133)"
        )
    };
    tcx.lint_node(
...
fn report_unused_unsafe(tcx: TyCtxt, used_unsafe: &FxHashSet<ast::NodeId>, id: ast::NodeId) {
    ...
    if let Some((kind, id)) = is_enclosed(tcx, used_unsafe, id) {
        db.span_label(
            tcx.sess.codemap().def_span(tcx.hir.span(id)),
...
fn builtin_derive_def_id<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) -> Option<DefId> {
    ...
    if let Some(impl_def_id) = tcx.impl_of_method(def_id) {
        if tcx.has_attr(impl_def_id, "automatically_derived") {
            ...
            debug!(
                "builtin_derive_def_id({:?}) - not automatically derived",
                def_id
            );
            None
        }
...
pub fn check_unsafety<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) {
    ...
    unsafe_blocks.sort();
    let used_unsafe: FxHashSet<_> = unsafe_blocks
        .iter()
