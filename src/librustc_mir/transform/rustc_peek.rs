...
use dataflow::move_paths::{LookupResult, MovePathIndex};
...
impl MirPass for SanityCheck {
    fn run_pass<'a, 'tcx>(&self, tcx: TyCtxt<'a, 'tcx, 'tcx>, src: MirSource, mir: &mut Mir<'tcx>) {
        ...
        if !tcx.has_attr(def_id, "rustc_mir_borrowck") {
            debug!(
                "skipping rustc_peek::SanityCheck on {}",
                tcx.item_path_str(def_id)
            );
            return;
        } else {
            debug!(
                "running rustc_peek::SanityCheck on {}",
                tcx.item_path_str(def_id)
            );
        }
    ...
    O: BitDenotation<Idx = MovePathIndex> + HasMoveData<'tcx>,
{
    ...
    let mut sets = dataflow::BlockSets {
        on_entry: &mut entry,
        gen_set: &mut gen,
        kill_set: &mut kill,
    };
    ...
    for (j, stmt) in statements.iter().enumerate() {
        ...
        let (lvalue, rvalue) = match stmt.kind {
            mir::StatementKind::Assign(ref lvalue, ref rvalue) => (lvalue, rvalue),
            ...
            mir::StatementKind::SetDiscriminant { .. } => span_bug!(
        ...
        debug!(
            "rustc_peek: computing effect on lvalue: {:?} ({:?}) in stmt: {:?}",
            lvalue,
            lhs_mpi,
            stmt
        );
        // reset GEN and KILL sets before emulating their effect.
        for e in sets.gen_set.words_mut() {
            *e = 0;
        }
        for e in sets.kill_set.words_mut() {
            *e = 0;
        }
