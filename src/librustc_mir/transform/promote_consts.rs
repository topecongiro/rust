...
impl<'tcx> Visitor<'tcx> for TempCollector<'tcx> {
    fn visit_local(&mut self, &index: &Local, context: LvalueContext<'tcx>, location: Location) {
...
impl<'a, 'tcx> Promoter<'a, 'tcx> {
    ...
    fn promote_candidate(mut self, candidate: Candidate) {
        ...
        let mut rvalue = match candidate {
            ...
            Candidate::ShuffleIndices(bb) => match self.source[bb].terminator_mut().kind {
...
pub fn promote_candidates<'a, 'tcx>(
    ...
    for candidate in candidates.into_iter().rev() {
        let (span, ty) = match candidate {
            ...
            Candidate::ShuffleIndices(bb) => {
                ...
                let ty = match terminator.kind {
                    TerminatorKind::Call { ref args, .. } => args[2].ty(mir, tcx),
                    _ => {
                        span_bug!(
        ...
        let initial_locals = iter::once(LocalDecl::new_return_pointer(ty, span)).collect();
