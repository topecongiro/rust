...
use dataflow::{self, state_for_location, MaybeStorageLive};
...
impl<'tcx> MutVisitor<'tcx> for RenameLocalVisitor {
    fn visit_local(&mut self, local: &mut Local, _: LvalueContext<'tcx>, _: Location) {
        if *local == self.from {
            *local = self.to;
        }
    }
...
fn locals_live_across_suspend_points<'a, 'tcx>(
    ...
    let storage_live =
        dataflow::do_dataflow(tcx, mir, node_id, &[], &dead_unwinds, analysis, |bd, p| {
            &bd.mir().local_decls[p]
        });
    ...
    let liveness = liveness::liveness_of_locals(
        mir,
        LivenessMode {
