...
use interpret::{self, const_val_field, const_variant_index};

use rustc::hir::def::{CtorKind, Def};
use rustc::hir::pat_util::EnumerateAndAdjustIterator;
use rustc::hir::{self, PatKind, RangeEnd};
use rustc::middle::const_val::ConstVal;
use rustc::mir::interpret::{ConstValue, GlobalId, Scalar, Value};
use rustc::mir::{fmt_const_val, BorrowKind, Field, Mutability};
...
impl<'tcx> fmt::Display for Pattern<'tcx> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self.kind {
            ...
            PatternKind::Constant { value } => print_const_val(value, f),
...
impl<'a, 'tcx> PatternContext<'a, 'tcx> {
    ...
    fn lower_pattern_unadjusted(&mut self, pat: &'tcx hir::Pat) -> Pattern<'tcx> {
        ...
        let kind = match pat.node {
            ...
            PatKind::Range(ref lo_expr, ref hi_expr, end) => {
                match (self.lower_lit(lo_expr), self.lower_lit(hi_expr)) {
                    (PatternKind::Constant { value: lo }, PatternKind::Constant { value: hi }) => {
                        use std::cmp::Ordering;
                        let cmp = compare_const_vals(self.tcx, lo, hi, self.param_env.and(ty));
                        match (end, cmp) {
            ...
            PatKind::Slice(ref prefix, ref slice, ref suffix) => {
                ...
                match ty.sty {
                    ty::TyRef(_, ty, _) => PatternKind::Deref {
                        subpattern: Pattern {
                            ty,
                            span: pat.span,
                            kind: Box::new(
            ...
            PatKind::Struct(ref qpath, ref fields, _) => {
                ...
                let subpatterns = fields
                    ...
                    .map(|field| FieldPattern {
                        field: Field::new(self.tcx.field_index(field.node.id, self.tables)),
    ...
    fn lower_path(&mut self, qpath: &hir::QPath, id: hir::HirId, span: Span) -> Pattern<'tcx> {
                ...
                match ty::Instance::resolve(self.tcx, self.param_env, def_id, substs) {
                    Some(instance) => {
                        ...
                        match self.tcx.at(span).const_eval(self.param_env.and(cid)) {
                            Ok(value) => return self.const_to_pat(instance, value, id, span),
                            Err(err) => {
                                err.report_as_error(
                                    self.tcx.at(span),
    ...
    fn const_to_pat(
        ...
        let kind = match cv.ty.sty {
            ...
            ty::TyAdt(adt_def, substs) if adt_def.is_enum() => match cv.val {
                ConstVal::Value(val) => {
                    let variant_index =
                        const_variant_index(self.tcx, self.param_env, instance, val, cv.ty)
            ...
            ty::TyTuple(fields) => PatternKind::Leaf {
                subpatterns: adt_subpatterns(fields.len(), None),
            },
            ty::TyArray(_, n) => PatternKind::Array {
            ...
            _ => PatternKind::Constant { value: cv },
        };
    ...
    if let ty::TyRef(_, rty, _) = ty.value.sty {
        if let ty::TyStr = rty.sty {
            match (a.to_byval_value(), b.to_byval_value()) {
                (
                    Some(Value::ScalarPair(Scalar::Ptr(ptr_a), len_a)),
                    Some(Value::ScalarPair(Scalar::Ptr(ptr_b), len_b)),
    ...
    let lit = match *lit {
        ...
        LitKind::Int(n, _) => {
            ...
            let n = match ity {
                ...
                Int::Signed(IntTy::I8) if neg => (n as i8).overflowing_neg().0 as u8 as u128,
                Int::Signed(IntTy::I16) if neg => (n as i16).overflowing_neg().0 as u16 as u128,
                Int::Signed(IntTy::I32) if neg => (n as i32).overflowing_neg().0 as u32 as u128,
                Int::Signed(IntTy::I64) if neg => (n as i64).overflowing_neg().0 as u64 as u128,
                Int::Signed(IntTy::I128) if neg => (n as i128).overflowing_neg().0 as u128,
            ...
            let defined = tcx
                ...
                .size
                .bits() as u8;
            ConstValue::Scalar(Scalar::Bits { bits: n, defined })
        }
        LitKind::Float(n, fty) => parse_float(n, fty, neg)?,
