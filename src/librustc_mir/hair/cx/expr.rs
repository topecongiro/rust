...
use rustc::ty::{self, AdtKind, Ty, VariantDef};
...
fn make_mirror_unadjusted<'a, 'gcx, 'tcx>(
    ...
    let kind = match expr.node {
        ...
        hir::ExprRepeat(ref v, count) => {
            ...
            let count = match cx.tcx
                .at(c.span)
                .const_eval(cx.param_env.and((def_id, substs)))
            {
                Ok(&ty::Const {
                    val: ConstVal::Integral(ConstInt::Usize(u)),
                    ..
                }) => u,
        ...
        hir::ExprTupField(ref source, index) => ExprKind::Field {
            lhs: source.to_ref(),
