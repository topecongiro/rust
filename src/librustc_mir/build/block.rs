...
impl<'a, 'gcx, 'tcx> Builder<'a, 'gcx, 'tcx> {
    ...
    fn update_visibility_scope_for_safety_mode(&mut self, span: Span, safety_mode: BlockSafety) {
        ...
        let new_unsafety = match safety_mode {
            ...
            BlockSafety::PopUnsafe => {
                self.push_unsafe_count = self.push_unsafe_count
                    .checked_sub(1)
        ...
        if let Some(unsafety) = new_unsafety {
            self.visibility_scope =
                self.new_visibility_scope(span, LintLevel::Inherited, Some(unsafety));
        }
    }
}
