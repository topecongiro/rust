    ...
    fn invalidate_cache(&mut self, storage_only: bool, this_scope_only: bool) {
        ...
        if !storage_only && !this_scope_only {
            for dropdata in &mut self.drops {
                if let DropKind::Value {
                    ref mut cached_block,
                } = dropdata.kind
                {
...
impl<'a, 'gcx, 'tcx> Builder<'a, 'gcx, 'tcx> {
        ...
        F: FnOnce(&mut Builder<'a, 'gcx, 'tcx>) -> BlockAnd<R>,
    {
        ...
        debug!(
            "in_scope: exiting opt_scope={:?} block={:?}",
            opt_scope,
            block
        );
        block.and(rv)
        ...
        F: FnOnce(&mut Builder<'a, 'gcx, 'tcx>) -> BlockAnd<R>,
    {
        ...
        if let LintLevel::Explicit(node_id) = lint_level {
            let same_lint_scopes = tcx.dep_graph.with_ignore(|| {
                ...
                let parent_hir_id = tcx.hir
                    .definitions()
                    .node_to_hir_id(self.visibility_scope_info[visibility_scope].lint_root);
                let current_hir_id = tcx.hir.definitions().node_to_hir_id(node_id);
                sets.lint_level_set(parent_hir_id) == sets.lint_level_set(current_hir_id)
            });

            if !same_lint_scopes {
                self.visibility_scope =
                    self.new_visibility_scope(region_scope.1.span, lint_level, None);
            }
        }
        self.push_scope(region_scope);
        let rv = unpack!(block = f(self));
        ...
        debug!(
            "in_scope: exiting region_scope={:?} block={:?}",
            region_scope,
            block
        );
        block.and(rv)
    ...
    pub fn pop_scope(
        ...
        let may_panic = self.scopes
            .last()
            .unwrap()
            .drops
            .iter()
            .any(|s| s.kind.may_panic());
        if may_panic {
        ...
        self.cfg
            .push_end_region(self.hir.tcx(), block, region_scope.1, scope.region_scope);
        unpack!(
    ...
    pub fn exit_scope(
        ...
        // If we are emitting a `drop` statement, we need to have the cached
        // diverge cleanup pads ready in case that drop panics.
        let may_panic = self.scopes[(len - scope_count)..]
            .iter()
...

        {
            ...
            while let Some((scope, rest_)) = { rest }.split_last_mut() {
                ...
                unpack!(
                    block =
                        build_scope_drops(&mut self.cfg, scope, rest, block, self.arg_count, false)
                );
            }
        }
    ...
    pub fn generator_drop_cleanup(&mut self) -> Option<BasicBlock> {
        ...
        while let Some((scope, rest_)) = { rest }.split_last_mut() {
            ...
            block = if let Some(b) = scope.cached_generator_drop {
                self.cfg
                    .terminate(block, src_info, TerminatorKind::Goto { target: b });
                return Some(result);
            } else {
                let b = self.cfg.start_new_block();
            ...
            // End all regions for scopes out of which we are breaking.
            self.cfg
                .push_end_region(self.hir.tcx(), block, src_info, scope.region_scope);

            unpack!(
                block = build_scope_drops(&mut self.cfg, scope, rest, block, self.arg_count, true)
            );
        }

        self.cfg
            .terminate(block, src_info, TerminatorKind::GeneratorDrop);

        Some(result)
    }

    /// Creates a new visibility scope, nested in the current one.
    pub fn new_visibility_scope(
        &mut self,
        span: Span,
        lint_level: LintLevel,
        ...
        debug!(
            ...
            safety,
            parent,
            self.visibility_scope_info.get(parent)
        );
        ...
        let scope_info = VisibilityScopeInfo {
            ...
            safety: safety.unwrap_or_else(|| self.visibility_scope_info[parent].safety),
        };
    ...
    pub fn topmost_scope(&self) -> region::Scope {
        self.scopes
            .last()
            .expect("topmost_scope: no scopes present")
            .region_scope
    }
    ...
    while let Some(drop_data) = iter.next() {
        ...
        match drop_data.kind {
            DropKind::Value { .. } => {
                ...
                let on_diverge = iter.clone()
                    .filter_map(|dd| match dd.kind {
...
fn build_diverge_scope<'a, 'gcx, 'tcx>(
    ...
    for (j, drop_data) in scope.drops.iter_mut().enumerate() {
        ...
        let cached_block = match drop_data.kind {
            DropKind::Value {
                ref mut cached_block,
            } => cached_block.ref_mut(generator_drop),
