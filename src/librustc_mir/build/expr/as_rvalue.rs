...
impl<'a, 'gcx, 'tcx> Builder<'a, 'gcx, 'tcx> {
    ...
    fn expr_as_rvalue(
        ...
        match expr.kind {
            ...
            ExprKind::Box { value } => {
                ...
                this.cfg
                    .push_assign(block, source_info, &Lvalue::Local(result), box_);
    ...
    pub fn build_binary_op(
        ...
        if self.hir.check_overflow() && op.is_checkable() && ty.is_integral() {
            ...
            let err = ConstMathErr::Overflow(match op {
                ...
                BinOp::Shl => Op::Shl,
                BinOp::Shr => Op::Shr,
                _ => bug!("MIR build_binary_op: {:?} is not checkable", op),
            });
            ...
            if ty.is_integral() && (op == BinOp::Div || op == BinOp::Rem) {
                ...
                block = self.assert(
                    block,
                    Operand::Move(is_zero),
                    false,
                    AssertMessage::Math(zero_err),
                    span,
                );
                ...
                if ty.is_signed() {
                    ...
                    block = self.assert(
                        ...
                        Operand::Move(of),
                        false,
                        AssertMessage::Math(overflow_err),
                        span,
                    );
                }
            }
    ...
    fn neg_1_literal(&mut self, span: Span, ty: Ty<'tcx>) -> Operand<'tcx> {
        let literal = match ty.sty {
            ...
            _ => span_bug!(span, "Invalid type for neg_1_literal: `{:?}`", ty),
        };
    ...
    fn minval_literal(&mut self, span: Span, ty: Ty<'tcx>) -> Operand<'tcx> {
        let literal = match ty.sty {
            ...
            _ => span_bug!(span, "Invalid type for minval_literal: `{:?}`", ty),
        };
