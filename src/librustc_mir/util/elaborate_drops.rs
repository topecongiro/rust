    ...
    D: DropElaborator<'b, 'tcx>,
{
    fn lvalue_ty(&self, lvalue: &Lvalue<'tcx>) -> Ty<'tcx> {
        lvalue
            .ty(self.elaborator.mir(), self.tcx())
            .to_ty(self.tcx())
    }
    ...
    pub fn elaborate_drop<'a>(&mut self, bb: BasicBlock) {
        ...
        match style {
            ...
            DropStyle::Static => {
                ...
                self.elaborator
                    .clear_drop_flag(loc, self.path, DropFlagMode::Deep);
    ...
    fn move_paths_for_fields(
        ...
        variant
            ...
            .map(|(i, f)| {
                ...
                let field_ty = self.tcx().normalize_associated_type_in_env(
    ...
    fn open_drop_for_box<'a>(&mut self, ty: Ty<'tcx>) -> BasicBlock {
        ...
        let unwind_succ = self.unwind
            .map(|unwind| self.box_free_block(ty, unwind, Unwind::InCleanup));
    ...
    fn open_drop_for_adt_contents(
        ...
        if !adt.is_enum() {
            let fields =
                self.move_paths_for_fields(self.lvalue, self.path, &adt.variants[0], substs);
    ...
    fn open_drop_for_multivariant(
        ...
        for (variant_index, discr) in adt.discriminants(self.tcx()).enumerate() {
            ...
            if let Some(variant_path) = subpath {
                let base_lv = self.lvalue
                    .clone()
                    .elem(ProjectionElem::Downcast(adt, variant_index));
    ...
    fn destructor_call_block<'a>(&mut self, (succ, unwind): (BasicBlock, Unwind)) -> BasicBlock {
    ...
    fn drop_loop(
        ...
        let loop_block = BasicBlockData {
            statements: vec![
                self.assign(
                    can_go,
                    Rvalue::BinaryOp(BinOp::Eq, copy(&Lvalue::Local(cur)), copy(length_or_end)),
                ),
            ],
            is_cleanup: unwind.is_cleanup(),
            terminator: Some(Terminator {
    ...
    fn open_drop_for_array(&mut self, ety: Ty<'tcx>) -> BasicBlock {
        ...
        let base_block = BasicBlockData {
            statements: vec![
                ...
                self.assign(
                    size_is_zero,
                    Rvalue::BinaryOp(BinOp::Eq, move_(size), self.constant_usize(0)),
                ),
            ],
            is_cleanup: self.unwind.is_cleanup(),
            terminator: Some(Terminator {
    ...
    fn drop_loop_pair(&mut self, ety: Ty<'tcx>, ptr_based: bool) -> BasicBlock {
        ...
        let succ = self.succ; // FIXME(#6393)
        let loop_block = self.drop_loop(succ, cur, &length_or_end, ety, unwind, ptr_based);
        ...
        if ptr_based {
            ...
            // cur = tmp as *mut T;
            // end = Offset(cur, len);
            drop_block_stmts.push(self.assign(
                &tmp,
                Rvalue::Ref(tcx.types.re_erased, BorrowKind::Mut, self.lvalue.clone()),
            ));
            drop_block_stmts.push(self.assign(
                &cur,
                Rvalue::Cast(CastKind::Misc, Operand::Move(tmp.clone()), iter_ty),
            ));
            drop_block_stmts.push(self.assign(
                &length_or_end,
                Rvalue::BinaryOp(
                    BinOp::Offset,
                    Operand::Copy(cur.clone()),
                    Operand::Move(length.clone()),
                ),
            ));
        } else {
    ...
    fn new_block<'a>(&mut self, unwind: Unwind, k: TerminatorKind<'tcx>) -> BasicBlock {
