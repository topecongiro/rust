...
impl<'cx, 'gcx, 'tcx> MirBorrowckCtxt<'cx, 'gcx, 'tcx> {
    ...
    fn check_for_invalidation_at_exit(
        ...
        if places_conflict(self.tcx, self.mir, place, root_place, sd) {
            ...
            self.report_borrowed_value_does_not_live_long_enough(context, borrow, span)
        }
    }
                    ...
                    Err(place_err) => {
                        ...
                        if place != place_err {
                            if let Some(name) = self.describe_place(place_err) {
                                err.note(&format!(
                                    "the value which is causing this path not to be \
                                     mutable is...: `{}`",
                                    name
                                ));
                            }
            ...
            Reservation(WriteKind::Mutate) | Write(WriteKind::Mutate) => {
                match self.is_mutable(place, is_local_mutation_allowed) {
                    ...
                    Err(place_err) => {
                        ...
                        let err_info = if let Place::Projection(box Projection {
                        ...
                        )) = err_info
                        {
                            ...
                            if place != place_err {
                                if let Some(name) = self.describe_place(place_err) {
                                    err.note(&format!(
                                        "the value which is causing this path not \
                                         to be mutable is...: `{}`",
                                        name
                                    ));
                                }
