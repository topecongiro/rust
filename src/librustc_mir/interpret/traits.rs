...
use rustc::mir::interpret::{EvalResult, Pointer, Scalar, Value};
...
impl<'a, 'mir, 'tcx, M: Machine<'mir, 'tcx>> EvalContext<'a, 'mir, 'tcx, M> {
    ...
    pub fn get_vtable(
        ...
        self.memory
            .write_ptr_sized_unsigned(vtable, ptr_align, drop.into())?;
        ...
        for (i, method) in methods.iter().enumerate() {
            if let Some((def_id, substs)) = *method {
                ...
                self.memory
                    .write_ptr_sized_unsigned(method_ptr, ptr_align, fn_ptr.into())?;
            }
        }

        self.memory
            .mark_static_initialized(vtable.alloc_id, Mutability::Immutable)?;
    ...
    pub fn read_size_and_align_from_vtable(
        ...
        let size = self
            .memory
            .read_ptr_sized(vtable.offset(pointer_size, self)?, pointer_align)?
            .to_bits(pointer_size)? as u64;
        let align = self
            .memory
            .read_ptr_sized(vtable.offset(pointer_size * 2, self)?, pointer_align)?
            .to_bits(pointer_size)? as u64;
        Ok((
