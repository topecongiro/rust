...
where
    F: FnMut(&mir::LvalueProjection<'tcx>) -> bool,
{
...
fn lvalue_contents_drop_state_cannot_differ<'a, 'gcx, 'tcx>(
    ...
    match ty.sty {
        ty::TyArray(..) | ty::TySlice(..) | ty::TyRef(..) | ty::TyRawPtr(..) => {
            debug!(
                "lvalue_contents_drop_state_cannot_differ lv: {:?} ty: {:?} refd => true",
                lv,
                ty
            );
            true
        ...
        ty::TyAdt(def, _) if (def.has_dtor(tcx) && !def.is_box()) || def.is_union() => {
            debug!(
                "lvalue_contents_drop_state_cannot_differ lv: {:?} ty: {:?} Drop => true",
                lv,
                ty
            );
            true
    ...
    F: FnMut(MovePathIndex),
{
    match lookup_result {
        ...
        LookupResult::Exact(e) => on_all_children_bits(tcx, mir, move_data, e, each_child),
    }
}
    ...
    F: FnMut(MovePathIndex),
{
    fn is_terminal_path<'a, 'gcx, 'tcx>(
        ...
        lvalue_contents_drop_state_cannot_differ(tcx, mir, &move_data.move_paths[path].lvalue)
    }
    ...
    F: FnMut(MovePathIndex, DropFlagState),
{
    ...
    for arg in mir.args_iter() {
        ...
        on_lookup_result_bits(tcx, mir, move_data, lookup_result, |mpi| {
    ...
    F: FnMut(MovePathIndex, DropFlagState),
{
    ...
    for_location_inits(tcx, mir, move_data, loc, |mpi| {
        callback(mpi, DropFlagState::Present)
    });
}
    ...
    for ii in &move_data.init_loc_map[loc] {
        ...
        match init.kind {
            InitKind::Deep => {
                ...
                on_all_children_bits(tcx, mir, move_data, path, &mut callback)
            }
            InitKind::Shallow => {
                let mpi = init.path;
                callback(mpi);
