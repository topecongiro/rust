...
impl<'o, 'gcx: 'tcx, 'tcx> AstConv<'gcx, 'tcx> + 'o {
    ...
    fn ast_type_binding_to_poly_projection_predicate(
        ...
        let assoc_ty = tcx
            ...
            .find(|i| i.kind == ty::AssociatedKind::Type && i.name.to_ident() == assoc_ident)
            ...
            dup_bindings
                ...
                .and_modify(|prev_span| {
                    let mut err = self.tcx().struct_span_lint_node(
                        ...
                        binding.span,
                        &format!(
                            "associated type binding `{}` specified more than once",
                            binding.item_name
                        ),
                    );
    ...
    pub fn ast_ty_to_ty(&self, ast_ty: &hir::Ty) -> Ty<'tcx> {
        ...
        let result_ty = match ast_ty.node {
            ...
            hir::TyTup(ref fields) => tcx.mk_tup(fields.iter().map(|t| self.ast_ty_to_ty(&t))),
            hir::TyBareFn(ref bf) => {
...
fn check_type_argument_count(tcx: TyCtxt, span: Span, supplied: usize, ty_params: ParamRange) {
    ...
    if supplied < required {
        ...
        struct_span_err!(
            ...
            format!(
                "{} {} type argument{}",
                expected, required, arguments_plural
            ),
        )
            .emit();
