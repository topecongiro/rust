...
pub fn impl_wf_check<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>) {
    ...
    tcx.hir
        .krate()
        .visit_all_item_likes(&mut ImplWfCheck { tcx: tcx });
}
...
fn enforce_impl_params_are_constrained<'a, 'tcx>(
    tcx: TyCtxt<'a, 'tcx, 'tcx>,
    ...
    let lifetimes_in_associated_types: FxHashSet<_> = impl_item_refs
        ...
        .flat_map(|def_id| ctp::parameters_for(&tcx.type_of(def_id), true))
        .collect();
    for (ty_lifetime, lifetime) in impl_generics
        .regions
        .iter()
...
fn enforce_impl_items_are_distinct<'a, 'tcx>(
    tcx: TyCtxt<'a, 'tcx, 'tcx>,
    impl_item_refs: &[hir::ImplItemRef],
) {
    ...
    for impl_item_ref in impl_item_refs {
        ...
        match seen_items.entry(impl_item.name) {
            Occupied(entry) => {
                ...
                err.span_label(
                    *entry.get(),
                    format!("previous definition of `{}` here", impl_item.name),
                );
