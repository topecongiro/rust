...
impl<'a, 'tcx> AstConv<'tcx, 'tcx> for ItemCtxt<'a, 'tcx> {
    ...
    fn re_infer(
        &self,
        _span: Span,
        _def: Option<&ty::RegionParameterDef>,
...
fn convert_item<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, item_id: ast::NodeId) {
    ...
    match it.node {
        ...
        hir::ItemExternCrate(_) | hir::ItemUse(..) | hir::ItemMod(_) | hir::ItemGlobalAsm(_) => {}
        hir::ItemForeignMod(ref foreign_mod) => for item in &foreign_mod.items {
    ...
    for variant in variants {
        ...
        prev_discr = Some(
            if let Some(e) = variant.node.disr_expr {
                ...
                let result = tcx.at(variant.span)
                    .const_eval(param_env.and((expr_did, substs)));
...
fn adt_def<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) -> &'tcx ty::AdtDef {
    ...
    let (kind, variants) = match item.node {
        ...
        ItemUnion(ref def, _) => (
            AdtKind::Union,
            vec![
                convert_struct_variant(tcx, def_id, item.name, ty::VariantDiscr::Relative(0), def),
            ],
        ),
        _ => bug!(),
    };
...
fn generics_of<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) -> &'tcx ty::Generics {
    ...
    let parent_def_id = match node {
        ...
        NodeExpr(&hir::Expr {
            node: hir::ExprClosure(..),
            ..
        }) => Some(tcx.closure_base_def_id(def_id)),
        NodeTy(&hir::Ty {
            node: hir::TyImplTraitExistential(..),
            ..
        }) => {
    ...
    let (ast_generics, opt_inputs) = match node {
        ...
        NodeTy(&hir::Ty {
            node: hir::TyImplTraitExistential(ref exist_ty, _),
            ..
    ...
    }) = node
    {
        ...
        tcx.with_freevars(node_id, |fv| {
            types.extend(fv.iter().zip(2..).map(|(_, i)| {
                ty::TypeParameterDef {
                    index: type_start + i,
    ...
    match tcx.hir.get(node_id) {
        ...
        NodeImplItem(item) => match item.node {
            ...
            ImplItemKind::Type(ref ty) => {
                if tcx.impl_trait_ref(tcx.hir.get_parent_did(node_id))
                    .is_none()
                {
                    span_err!(
                        tcx.sess,
        ...
        NodeItem(item) => match item.node {
            ...
            | ItemConst(ref t, _)
            | ItemTy(ref t, _)
            | ItemImpl(.., ref t, _) => icx.to_ty(t),
            ItemFn(..) => {
                let substs = Substs::identity_for_item(tcx, def_id);
                tcx.mk_fn_def(def_id, substs)
            }
            ItemEnum(..) | ItemStruct(..) | ItemUnion(..) => {
                let def = tcx.adt_def(def_id);
                let substs = Substs::identity_for_item(tcx, def_id);
                tcx.mk_adt(def, substs)
            }
            ItemAutoImpl(..) | ItemTrait(..) | ItemMod(..) | ItemForeignMod(..)
            | ItemGlobalAsm(..) | ItemExternCrate(..) | ItemUse(..) => {
                span_bug!(
                    item.span,
        ...
        | NodeVariant(&Spanned {
            node: hir::Variant_ { data: ref def, .. },
            ..
        }) => match *def {
        ...
        NodeExpr(&hir::Expr {
            node: hir::ExprClosure(.., is_generator),
            ..
        }) => {
            if is_generator {
                let hir_id = tcx.hir.node_to_hir_id(node_id);
...
fn fn_sig<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) -> ty::PolyFnSig<'tcx> {
    ...
    match tcx.hir.get(node_id) {
        ...
        NodeItem(&hir::Item {
            node: ItemFn(ref decl, unsafety, _, abi, _, _),
            ..
        }) => AstConv::ty_of_fn(&icx, unsafety, abi, decl),

        NodeForeignItem(&hir::ForeignItem {
            node: ForeignItemFn(ref fn_decl, _, _),
            ..
        }) => {
        ...
        | NodeVariant(&Spanned {
            ...
            ty::Binder(tcx.mk_fn_sig(inputs, ty, false, hir::Unsafety::Normal, abi::Abi::Rust))
        }
...
fn impl_trait_ref<'a, 'tcx>(
    ...
    match tcx.hir.expect_item(node_id).node {
        hir::ItemAutoImpl(_, ref ast_trait_ref) => Some(AstConv::instantiate_mono_trait_ref(
            &icx,
            ast_trait_ref,
            tcx.mk_self_type(),
        )),
        hir::ItemImpl(.., ref opt_trait_ref, _, _) => opt_trait_ref.as_ref().map(|ast_trait_ref| {
...
fn early_bound_lifetimes_from_generics<'a, 'tcx>(
    ...
    ast_generics.lifetimes.iter().filter(move |l| {
...
fn explicit_predicates_of<'a, 'tcx>(
    ...
    let (ast_generics, opt_inputs) = match node {
        ...
        NodeItem(item) => match item.node {
            ...
            | ItemEnum(_, ref generics)
            | ItemStruct(_, ref generics)
            | ItemUnion(_, ref generics) => (generics, None),
        ...
        NodeTy(&Ty {
            node: TyImplTraitExistential(ref exist_ty, _),
            span,
            ..
        }) => {
            let substs = Substs::identity_for_item(tcx, def_id);
            ...
            let bounds = compute_bounds(&icx, anon_ty, &exist_ty.bounds, SizedByDefault::Yes, span);
    ...
    for info in univ_impl_trait_info.iter() {
        ...
        index += 1;
        let bounds = compute_bounds(&icx, param_ty, info.bounds, SizedByDefault::Yes, info.span);
        predicates.extend(bounds.predicates(tcx, param_ty));
    }
...
fn predicates_from_bound<'tcx>(
    ...
    match *bound {
        ...
        hir::TraitTyParamBound(_, hir::TraitBoundModifier::Maybe) => Vec::new(),
    }
}
...
fn is_auto_impl<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) -> bool {
    match tcx.hir.get_if_local(def_id) {
        Some(hir_map::NodeItem(&hir::Item {
            node: hir::ItemAutoImpl(..),
            ..
        })) => true,
        Some(_) => false,
...
/// arguments
fn extract_universal_impl_trait_info<'a, 'tcx>(
    tcx: TyCtxt<'a, 'tcx, 'tcx>,
    ...
    visitor
        ...
        .map(|ty| {
            if let hir::TyImplTraitUniversal(_, ref bounds) = ty.node {
                ...
                span_bug!(
                    ty.span,
                    "this type should be a universally quantified impl trait. this is a bug"
                )
            }
        })
        .collect()
