...
use rustc::ty::TypeVariants::{TyAdt, TyArray, TyChar, TyNever, TyRef, TyStr, TyTuple, TyUint};
...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
    ...
    pub fn check_binop(
        ...
        debug!(
            ...
            expr.id, expr, op, lhs_expr, rhs_expr
        );
    ...
    fn check_overloaded_binop(
        ...
        debug!(
            "check_overloaded_binop(expr.id={}, op={:?}, is_assign={:?})",
            expr.id, op, is_assign
        );
                    ...
                    match is_assign {
                        IsAssign::Yes => {
                            ...
                            if let TyRef(_, mut rty, _) = lhs_ty.sty {
                                if {
                                    !self.infcx.type_moves_by_default(
                                        self.param_env,
                                        rty,
                                        lhs_expr.span,
                                    )
                            ...
                            if let Some(missing_trait) = missing_trait {
                                if op.node == hir::BiAdd
                                    && self.check_str_addition(
                                        expr, lhs_expr, rhs_expr, lhs_ty, rhs_ty, &mut err,
                                    ) {
    ...
    fn check_str_addition(
        ...
        match (&lhs_ty.sty, &rhs_ty.sty) {
            (&TyRef(_, l_ty, _), &TyRef(_, r_ty, _)) if l_ty.sty == TyStr && r_ty.sty == TyStr => {
                err.span_label(
                    expr.span,
                ...
                match codemap.span_to_snippet(lhs_expr.span) {
                    Ok(lstring) => {
                        err.span_suggestion(lhs_expr.span, msg, format!("{}.to_owned()", lstring))
                    }
                    _ => err.help(msg),
                };
                true
    ...
    fn lookup_op_method(
        ...
        let (opname, trait_did) = if let Op::Binary(op, IsAssign::Yes) = op {
            ...
            match op.node {
                ...
                hir::BiAnd | hir::BiOr => span_bug!(span, "&& and || are not overloadable"),
            }
...
impl BinOpCategory {
    fn from(op: hir::BinOp) -> BinOpCategory {
        match op.node {
            hir::BiShl | hir::BiShr => BinOpCategory::Shift,

            hir::BiAdd | hir::BiSub | hir::BiMul | hir::BiDiv | hir::BiRem => BinOpCategory::Math,

            hir::BiBitXor | hir::BiBitAnd | hir::BiBitOr => BinOpCategory::Bitwise,

            hir::BiEq | hir::BiNe | hir::BiLt | hir::BiLe | hir::BiGe | hir::BiGt => {
                BinOpCategory::Comparison
            }

            hir::BiAnd | hir::BiOr => BinOpCategory::Shortcircuit,
        }
    }
}
