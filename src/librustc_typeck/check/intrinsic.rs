...
pub fn check_intrinsic_type<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, it: &hir::ForeignItem) {
    ...
    let (n_tps, inputs, output) = if name.starts_with("atomic_") {
        ...
        let (n_tps, inputs, output) = match split[1] {
            ...
            "store" => (1, vec![tcx.mk_mut_ptr(param(0)), param(0)], tcx.mk_nil()),

            "xchg" | "xadd" | "xsub" | "and" | "nand" | "or" | "xor" | "max" | "min" | "umax"
            | "umin" => (1, vec![tcx.mk_mut_ptr(param(0)), param(0)], param(0)),
            "fence" | "singlethreadfence" => (0, Vec::new(), tcx.mk_nil()),
            op => {
                struct_span_err!(
                    tcx.sess,
                    it.span,
        ...
        let (n_tps, inputs, output) = match &name[..] {
            ...
            "size_of_val" | "min_align_of_val" => (
                1,
                vec![tcx.mk_imm_ref(
                    tcx.mk_region(ty::ReLateBound(ty::DebruijnIndex::INNERMOST, ty::BrAnon(0))),
                    param(0),
                )],
                tcx.types.usize,
            ),
            ...
            "move_val_init" => (1, vec![tcx.mk_mut_ptr(param(0)), param(0)], tcx.mk_nil()),
            "prefetch_read_data"
            | "prefetch_write_data"
            ...
            "drop_in_place" => (1, vec![tcx.mk_mut_ptr(param(0))], tcx.mk_nil()),
            ...
            "volatile_load" => (1, vec![tcx.mk_imm_ptr(param(0))], param(0)),
            "volatile_store" => (1, vec![tcx.mk_mut_ptr(param(0)), param(0)], tcx.mk_nil()),
            ...
            "unchecked_shl" | "unchecked_shr" => (1, vec![param(0), param(0)], param(0)),
            ...
            "nontemporal_store" => (1, vec![tcx.mk_mut_ptr(param(0)), param(0)], tcx.mk_nil()),
...
pub fn check_platform_intrinsic_type<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, it: &hir::ForeignItem) {
    ...
    let (n_tps, inputs, output) = match &*name {
        ...
        "simd_add" | "simd_sub" | "simd_mul" | "simd_rem" | "simd_div" | "simd_shl"
        | "simd_shr" | "simd_and" | "simd_or" | "simd_xor" | "simd_fmin" | "simd_fmax"
        | "simd_fpow" => (1, vec![param(0), param(0)], param(0)),
        ...
        "simd_scatter" => (3, vec![param(0), param(1), param(2)], tcx.mk_nil()),
    ...
    equate_intrinsic_type(tcx, it, n_tps, Abi::PlatformIntrinsic, inputs, output)
}
...
fn match_intrinsic_type_to_type<'a, 'tcx>(
    ...
    match *expected {
        ...
        Integer(signed, bits, _llvm_width) => match (signed, bits, &t.sty) {
            ...
            _ => simple_error(
                &format!("`{}`", t),
                &format!("`{}{n}`", if signed { "i" } else { "u" }, n = bits),
            ),
        },
        Float(bits) => match (bits, &t.sty) {
            (32, &ty::TyFloat(ast::FloatTy::F32)) | (64, &ty::TyFloat(ast::FloatTy::F64)) => {}
            _ => simple_error(&format!("`{}`", t), &format!("`f{n}`", n = bits)),
        },
        Pointer(ref inner_expected, ref _llvm_type, const_) => match t.sty {
        ...
        Vector(ref inner_expected, ref _llvm_type, len) => {
            ...
            if len as usize != t_len {
                simple_error(
                    &format!("vector with length {}", t_len),
                    &format!("length {}", len),
                );
                return;
            }
        ...
        Aggregate(_flatten, ref expected_contents) => match t.sty {
            ty::TyTuple(contents) => {
                if contents.len() != expected_contents.len() {
                    simple_error(
                        &format!("tuple with length {}", contents.len()),
                        &format!("tuple with length {}", expected_contents.len()),
                    );
                    return;
                }
                for (e, c) in expected_contents.iter().zip(contents) {
                    match_intrinsic_type_to_type(tcx, position, span, structural_to_nominal, e, c)
                }
            }
            _ => simple_error(&format!("`{}`", t), "tuple"),
        },
    }
}
