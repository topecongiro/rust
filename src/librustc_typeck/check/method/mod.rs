...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
    ...
    pub fn lookup_method(
        ...
        debug!(
            ...
            segment.name, self_ty, call_expr, self_expr
        );
        ...
        self.tcx
            .check_stability(pick.item.def_id, Some(call_expr.id), span);
    ...
    pub fn lookup_method_in_trait(
        ...
        let method_item = self
            .associated_item(trait_def_id, m_name, Namespace::Value)
            .unwrap();
        ...
        let fn_sig = self
            .replace_late_bound_regions_with_fresh_var(span, infer::FnCall, &fn_sig)
            .0;
    ...
    pub fn associated_item(
        ...
        self.tcx.associated_items(def_id).find(|item| {
            Namespace::from(item.kind) == ns && self.tcx.hygienic_eq(item_name, item.name, def_id)
        })
    }
}
