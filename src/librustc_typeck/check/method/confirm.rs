...
use astconv::AstConv;
use check::{callee, FnCtxt, Needs, PlaceOp};
...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
    pub fn confirm_method(
        ...
        debug!(
            ...
            unadjusted_self_ty, pick, segment.parameters,
        );
...
impl<'a, 'gcx, 'tcx> ConfirmContext<'a, 'gcx, 'tcx> {
    ...
    fn instantiate_method_substs(
        ...
        self.fcx
            .check_path_parameter_count(self.span, &mut fn_segment, true, supress_mismatch);
            ...
            if i < parent_substs.len() {
                ...
                match param.kind {
                    GenericParamDefKind::Lifetime => {
                        if let Some(lifetime) = provided
                            .as_ref()
                            .and_then(|p| p.lifetimes.get(i - parent_substs.len()))
                        {
                            return AstConv::ast_region_to_region(self.fcx, lifetime, Some(param))
                                .into();
                        }
                    }
    ...
    fn unify_receivers(&mut self, self_ty: Ty<'tcx>, method_self_ty: Ty<'tcx>) {
        match self
            .at(&self.misc(self.span), self.param_env)
            ...
            .get_mut(base_expr.hir_id)
        {
            ...
            for adjustment in &mut adjustments[..] {
                if let Adjust::Borrow(AutoBorrow::Ref(..)) = adjustment.kind {
                    debug!(
                        "convert_place_op_to_mutable: converting autoref {:?}",
                        adjustment
                    );
                    let mutbl = match mutbl {
                        hir::MutImmutable => AutoBorrowMutability::Immutable,
                    ...
                    adjustment.target = self.tcx.mk_ref(
                        region,
                        ty::TypeAndMut {
                            ty: source,
                            mutbl: mutbl.into(),
    ...
    fn predicates_require_illegal_sized_bound(
        ...
        traits::elaborate_predicates(self.tcx, predicates.predicates.clone())
            ...
            .any(|trait_pred| match trait_pred.skip_binder().self_ty().sty {
                ty::TyDynamic(..) => true,
                _ => false,
            })
    }
