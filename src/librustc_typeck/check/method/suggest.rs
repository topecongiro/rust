                ...
                self.autoderef(span, ty).any(|(ty, _)| {
                    self.probe(|_| {
                        let fn_once_substs = tcx.mk_substs_trait(
                            ty,
                            &[self
                                .next_ty_var(TypeVariableOrigin::MiscVariable(span))
                                .into()],
                        );
    ...
    pub fn report_method_error(
                ...
                match *source {
                    ...
                    CandidateSource::TraitSource(trait_did) => {
                        ...
                        err.help(&format!(
                            ...
                            args.map(|arg| arg
                                .iter()
                                .map(|arg| print::to_string(print::NO_ANN, |s| s.print_expr(arg)))
                                .collect::<Vec<_>>()
                                .join(", "))
                                .unwrap_or("...".to_owned())
                        ));
        ...
        match error {
            MethodError::NoMatch(NoMatchData {
                ...
                let item_kind = if is_method {
                    ...
                    if let TyAdt(ref adt_def, _) = actual.sty {
                        ...
                        suggestion = find_best_match_for_name(names, &item_name.as_str(), None);
                    }
                    "variant"
                } else {
                ...
                let mut err = if !actual.references_error() {
                    ...
                    // ({integer}/{float}).
                    let mut candidates = all_traits(self.tcx).into_iter().filter(|info| {
                        self.associated_item(info.def_id, item_name, Namespace::Value)
                            .is_some()
                    });
                    if let (true, false, Some(expr), Some(_)) = (
                        actual.is_numeric(),
                            ...
                            hir::ExprPath(ref qpath) => {
                                ...
                                if let &hir::QPath::Resolved(_, ref path) = &qpath {
                                    if let hir::def::Def::Local(node_id) = path.def {
                                        ...
                                        let snippet =
                                            tcx.sess.codemap().span_to_snippet(span).unwrap();
                                        err.span_suggestion(
                                            span,
                                            &format!(
                ...
                if !unsatisfied_predicates.is_empty() {
                    let bound_list = unsatisfied_predicates
                        .iter()
            ...
            MethodError::IllegalSizedBound(candidates) => {
                let msg = format!(
                    "the `{}` method cannot be invoked on a trait object",
                    item_name
                );
        ...
        if let Some(span) = span {
            ...
            for (i, trait_did) in candidates.iter().take(limit).enumerate() {
                if candidates.len() > 1 {
                    ...
                    msg.push_str(&format!("\n`use {};`", self.tcx.item_path_str(*trait_did)));
                }
            }
    ...
    fn type_derefs_to_local(
        ...
        fn is_local(ty: Ty) -> bool {
            match ty.sty {
                ...
                ty::TyDynamic(ref tr, ..) => {
                    tr.principal().map_or(false, |p| p.def_id().is_local())
                }
...
pub fn all_traits<'a, 'gcx, 'tcx>(tcx: TyCtxt<'a, 'gcx, 'tcx>) -> Vec<TraitInfo> {
    tcx.all_traits(LOCAL_CRATE)
        .iter()
        .map(|&def_id| TraitInfo { def_id })
        .collect()
}
...
impl<'a, 'tcx, 'gcx> hir::intravisit::Visitor<'tcx> for UsePlacementFinder<'a, 'tcx, 'gcx> {
    fn visit_mod(&mut self, module: &'tcx hir::Mod, _: Span, node_id: ast::NodeId) {
