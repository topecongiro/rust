    ...
    fn record(
        ...
        let live_across_yield = scope.map_or(Some(DUMMY_SP), |s| {
            self.region_scope_tree
                .yield_in_scope(s)
                .and_then(|(yield_span, expr_count)| {
...
pub fn resolve_interior<'a, 'gcx, 'tcx>(
    ...
    let type_list = fcx
        .tcx
        .fold_regions(&type_list, &mut false, |_, current_depth| {
            counter += 1;
            fcx.tcx
                .mk_region(ty::ReLateBound(current_depth, ty::BrAnon(counter)))
        });
    ...
    match fcx
        .at(&fcx.misc(body.value.span), fcx.param_env)
        .eq(interior, witness)
    {
