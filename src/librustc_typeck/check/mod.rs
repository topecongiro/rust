...
use rustc::ty::{self, GenericParamDefKind, RegionKind, ToPredicate, Ty, TyCtxt, Visibility};
use std::slice;

use lint;
use require_c_abi_if_variadic;
use session::{config, CompileIncomplete, Session};
...
use rustc::hir::{self, Item_, PatKind};
...
fn used_trait_imports<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) -> Lrc<DefIdSet> {
    ...
    fn visit_local(&mut self, local: &'gcx hir::Local) {
        let o_ty = match local.ty {
            Some(ref ty) => {
                ...
                debug!(
                    "visit_local: ty.hir_id={:?} o_ty={:?} c_ty={:?}",
                    ty.hir_id, o_ty, c_ty
                );
                self.fcx
                    .tables
        ...
        if panic_impl_did == fcx.tcx.hir.local_def_id(fn_id) {
            if let Some(panic_info_did) = fcx.tcx.lang_items().panic_info() {
                if declared_ret_ty.sty != ty::TyNever {
                    fcx.tcx
                        .sess
                        .span_err(decl.output.span(), "return type should be `!`");
                }
                ...
                if inputs.len() == 1 {
                    ...
                    fcx.tcx
                        .sess
                        .span_err(span, "function should have one argument");
                }
            } else {
                fcx.tcx
                    .sess
...
fn check_const_is_u8_array<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId, span: Span) {
    match tcx.type_of(def_id).sty {
        ty::TyArray(t, _) => match t.sty {
    ...
    tcx.sess
        .span_err(span, "must be an array of bytes like `[u8; N]`");
}
...
impl<'a, 'gcx, 'tcx> AstConv<'gcx, 'tcx> for FnCtxt<'a, 'gcx, 'tcx> {
    ...
    fn re_infer(&self, span: Span, def: Option<&ty::GenericParamDef>) -> Option<ty::Region<'tcx>> {
        let v = match def {
    ...
    fn ty_infer_for_def(&self, ty_param_def: &ty::GenericParamDef, span: Span) -> Ty<'tcx> {
...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
    ...
    fn instantiate_anon_types_from_return_value<T: TypeFoldable<'tcx>>(
        ...
        let (value, anon_type_map) = self.register_infer_ok_obligations(
            self.instantiate_anon_types(fn_def_id, self.body_id, self.param_env, value),
        );
    ...
    fn is_place_expr(&self, expr: &hir::Expr) -> bool {
        match expr.node {
            hir::ExprPath(hir::QPath::Resolved(_, ref path)) => match path.def {
                Def::Local(..) | Def::Upvar(..) | Def::Static(..) | Def::Err => true,
                _ => false,
            },

            hir::ExprType(ref e, _) => self.is_place_expr(e),

            hir::ExprUnary(hir::UnDeref, _) | hir::ExprField(..) | hir::ExprIndex(..) => true,
    ...
    fn resolve_place_op(&self, op: PlaceOp, is_mut: bool) -> (Option<DefId>, Symbol) {
        let (tr, name) = match (op, is_mut) {
            (PlaceOp::Deref, false) => (self.tcx.lang_items().deref_trait(), "deref"),
            (PlaceOp::Deref, true) => (self.tcx.lang_items().deref_mut_trait(), "deref_mut"),
            (PlaceOp::Index, false) => (self.tcx.lang_items().index_trait(), "index"),
            (PlaceOp::Index, true) => (self.tcx.lang_items().index_mut_trait(), "index_mut"),
        };
    ...
    fn check_argument_types(
        ...
        if variadic {
            fn variadic_error<'tcx>(s: &Session, span: Span, t: Ty<'tcx>, cast_ty: &str) {
                use structured_errors::{StructuredDiagnostic, VariadicError};
                ...
                ty::TyAdt(base_def, substs) if !base_def.is_enum() => {
                    ...
                    if let Some(index) = fields.iter().position(|f| f.ident.modern() == ident) {
                        ...
                        if field.vis.is_accessible_from(def_scope, self.tcx) {
                            ...
                            self.tcx
                                .check_stability(field.did, Some(expr.id), expr.span);
                            return field_ty;
                        }
            ...
            if !expr_t.is_primitive_ty() {
                ...
                match expr_t.sty {
                    ty::TyAdt(def, _) if !def.is_enum() => {
                        if let Some(suggested_field_name) = Self::suggest_field_name(
    ...
    fn available_field_names(&self, variant: &'tcx ty::VariantDef) -> Vec<ast::Name> {
        ...
        for field in variant.fields.iter() {
            let def_scope = self
                .tcx
                .adjust_ident(field.ident, variant.did, self.body_id)
                .1;
            if field.vis.is_accessible_from(def_scope, self.tcx) {
    ...
    fn check_expr_kind(
        ...
        match expr.node {
            hir::ExprBox(ref subexpr) => {
                let expected_inner = expected.to_option(self).map_or(NoExpectation, |ty| match ty
                    .sty
                {
            ...
            hir::ExprUnary(unop, ref oprnd) => {
                ...
                let mut oprnd_t =
                    self.check_expr_with_expectation_and_needs(&oprnd, expected_inner, needs);
                    ...
                    match unop {
                        hir::UnDeref => {
                            if let Some(mt) = oprnd_t.builtin_deref(true) {
                                ...
                                self.try_overloaded_deref(expr.span, oprnd_t, needs)
                            {
                                let method = self.register_infer_ok_obligations(ok);
            ...
            hir::ExprBlock(ref body, _) => self.check_block_with_expected(&body, expected),
            ...
            hir::ExprRepeat(ref element, ref count) => {
                ...
                let instance =
                    ty::Instance::resolve(tcx.global_tcx(), param_env, count_def_id, substs)
                        .unwrap();
            ...
            hir::ExprField(ref base, field) => self.check_field(expr, needs, &base, field),
    ...
    fn consider_hint_about_removing_semicolon(
        ...
        if self
            .can_sub(self.param_env, last_expr_ty, expected_ty)
            .is_err()
        {
            return;
        }
    ...
    pub fn instantiate_value_path(
        ...
        let (fn_start, has_self) = match (type_segment, fn_segment) {
            ...
            (Some((_, generics)), None) => (generics.params.len(), generics.has_self),
            (None, None) => (0, false),
        };
        ...
        debug!(
            ...
            node_id, ty_substituted
        );
        self.write_substs(self.tcx.hir.node_to_hir_id(node_id), substs);
        ty_substituted
    }

    fn check_rustc_args_require_const(&self, def_id: DefId, node_id: ast::NodeId, span: Span) {
        ...
        match self.tcx.hir.get(self.tcx.hir.get_parent_node(node_id)) {
            Node::NodeExpr(expr) => match expr.node {
...
pub fn check_bounds_are_used<'a, 'tcx>(
    ...
    debug!(
        ...
        generics.ty_params().count(),
        ty
    );

    // make a vector of booleans initially false, set to true when used
    if generics.ty_params().next().is_none() {
        return;
    }
