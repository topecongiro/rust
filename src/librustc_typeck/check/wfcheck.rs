...
use rustc::ty::{self, GenericParamDefKind, Lift, Ty, TyCtxt};
...
fn check_where_clauses<'a, 'gcx, 'fcx, 'tcx>(
    ...
    for &pred in predicates.predicates.iter() {
        ...
        let mut param_count = CountParams {
            params: FxHashSet(),
        };
...
fn check_method_receiver<'fcx, 'gcx, 'tcx>(
    ...
    let self_arg_ty = fcx
        .tcx
        .liberate_late_bound_regions(method.def_id, &ty::Binder::bind(self_arg_ty));
    ...
    if !fcx.tcx.features().arbitrary_self_types {
        match self_kind {
            ExplicitSelf::ByValue | ExplicitSelf::ByReference(_, _) | ExplicitSelf::ByBox => (),
