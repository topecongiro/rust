...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
    ...
    fn try_overloaded_call_traits(
        ...
        for &(opt_trait_def_id, method_name, borrow) in &[
            ...
            (
                ...
                Symbol::intern("call_mut"),
                true,
            ),
            (
                self.tcx.lang_items().fn_once_trait(),
                Symbol::intern("call_once"),
                false,
            ),
        ] {
            ...
            match self.lookup_method_in_trait(
                call_expr.span,
                method_name,
                trait_def_id,
                adjusted_ty,
    ...
    fn confirm_builtin_call(
        ...
        let fn_sig = self
            .replace_late_bound_regions_with_fresh_var(call_expr.span, infer::FnCall, &fn_sig)
            .0;
...
impl<'a, 'gcx, 'tcx> DeferredCallResolution<'gcx, 'tcx> {
    pub fn resolve(self, fcx: &FnCtxt<'a, 'gcx, 'tcx>) {
        ...
        assert!(
            fcx.closure_kind(self.closure_def_id, self.closure_substs)
                .is_some()
        );
