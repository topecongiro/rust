...
use errors::{CodeMapper, DiagnosticBuilder};
use rustc::hir;
use rustc::hir::def::Def;
use rustc::hir::map::{NodeExpr, NodeItem};
use rustc::hir::{print, Item, ItemConst};
use rustc::ty::adjustment::AllowTwoPhase;
use rustc::ty::{self, AssociatedItem, Ty};
use syntax::ast;
...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
    ...
    pub fn demand_suptype(&self, sp: Span, expected: Ty<'tcx>, actual: Ty<'tcx>) {
        self.demand_suptype_diag(sp, expected, actual)
            .map(|mut e| e.emit());
    }
    ...
    pub fn demand_coerce_diag(
        ...
        if let Some((sp, msg, suggestion)) = self.check_ref(expr, checked_ty, expected) {
            ...
            if let Ok(expr_text) = self.tcx.sess.codemap().span_to_snippet(expr.span) {
                let suggestions = iter::repeat(expr_text)
                    .zip(methods.iter())
    ...
    fn has_no_input_arg(&self, method: &AssociatedItem) -> bool {
        match method.def() {
            Def::Method(def_id) => self.tcx.fn_sig(def_id).inputs().skip_binder().len() == 1,
            _ => false,
        }
    }
        ...
        if let hir::ExprPath(hir::QPath::Resolved(_, ref path)) = expr.node {
            if let hir::def::Def::Local(id) = path.def {
                ...
                })) = self.tcx.hir.find(parent)
                {
                    let parent = self.tcx.hir.get_parent_node(*id);
                    if let (
                        Some(NodeExpr(hir::Expr {
                            node: hir::ExprMethodCall(path, span, expr),
                            ..
                        })),
                        1,
                    ) = (self.tcx.hir.find(parent), decl.inputs.len())
                    {
    ...
    fn check_ref(
        ...
        match (&expected.sty, &checked_ty.sty) {
            (&ty::TyRef(_, exp, _), &ty::TyRef(_, check, _)) => match (&exp.sty, &check.sty) {
                (&ty::TyStr, &ty::TyArray(arr, _)) | (&ty::TyStr, &ty::TySlice(arr))
                ...
                (&ty::TyArray(arr, _), &ty::TyStr) | (&ty::TySlice(arr), &ty::TyStr)
                    if arr == self.tcx.types.u8 =>
                {
                    if let hir::ExprLit(_) = expr.node {
                        let sp = self.sess().codemap().call_span_if_macro(expr.span);
                        if let Ok(src) = self.tcx.sess.codemap().span_to_snippet(sp) {
                            return Some((sp, "consider adding a leading `b`", format!("b{}", src)));
                        }
                    }
                }
                _ => {}
            ...
            (&ty::TyRef(_, _, mutability), _) => {
                ...
                let ref_ty = match mutability {
                    hir::Mutability::MutMutable => self
                        .tcx
                        .mk_mut_ref(self.tcx.mk_region(ty::ReStatic), checked_ty),
                    hir::Mutability::MutImmutable => self
                        .tcx
                        .mk_imm_ref(self.tcx.mk_region(ty::ReStatic), checked_ty),
                };
                if self.can_coerce(ref_ty, expected) {
                    // Use the callsite's span if this is a macro call. #41858
                    let sp = self.sess().codemap().call_span_if_macro(expr.span);
                    if let Ok(src) = self.tcx.sess.codemap().span_to_snippet(sp) {
                        let sugg_expr = match expr.node {
            ...
            (_, &ty::TyRef(_, checked, _)) => {
                ...
                if self
                    .infcx
                    .can_sub(self.param_env, checked, &expected)
                    .is_ok() && expr.span.ctxt().outer().expn_info().is_none()
                {
                    match expr.node {
                        // Maybe remove `&`?
                        ...
                        _ => {
                            if !self
                                .infcx
                                .type_moves_by_default(self.param_env, checked, expr.span)
                            {
                                let sp = self.sess().codemap().call_span_if_macro(expr.span);
            ...
            match (&expected_ty.sty, &checked_ty.sty) {
                ...
                (&ty::TyUint(_), &ty::TyFloat(_)) | (&ty::TyInt(_), &ty::TyFloat(_)) => {
                    if can_cast {
                        ...
                        err.warn(
                            "casting here will cause undefined behavior if the rounded value \
