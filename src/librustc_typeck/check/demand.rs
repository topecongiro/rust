...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
    ...
    pub fn demand_coerce_diag(
        ...
        if let Some(suggestion) = self.check_ref(expr, checked_ty, expected) {
            err.help(&suggestion);
        } else {
            let mode = probe::Mode::MethodCall;
            let suggestions = self.probe_for_return_type(
                syntax_pos::DUMMY_SP,
                mode,
                expected,
                checked_ty,
    ...
    fn get_best_match(&self, methods: &[AssociatedItem]) -> Vec<String> {
        let no_argument_methods: Vec<_> = methods
            .iter()
        ...
        match (&expected.sty, &checked_ty.sty) {
            ...
            (&ty::TyRef(_, mutability), _) => {
                ...
                let ref_ty = match mutability.mutbl {
                    hir::Mutability::MutMutable => self.tcx
                        .mk_mut_ref(self.tcx.mk_region(ty::ReStatic), checked_ty),
                    hir::Mutability::MutImmutable => self.tcx
            ...
            (_, &ty::TyRef(_, checked)) => {
                ...
                if self.infcx
                    .can_sub(self.param_env, checked.ty, &expected)
                    .is_ok() && expr.span.ctxt().outer().expn_info().is_none()
                {
                    match expr.node {
                        ...
                        _ => {
                            if !self.infcx.type_moves_by_default(
                                self.param_env,
                                checked.ty,
                                expr.span,
                            ) {
